new Vue({
    ...
    methods: {
        getTableData: function (currentIndex) {
            if (this.actId === '') {
                return this.showDialog('', 'info', false, '请填写活动ID')
            }
            var params = {};
            var _this = this;
            params.actId = this.actId;
            params.startDay = this.startDay;
            params.endDay = this.endDay;

            params.pageNo = currentIndex + 1;
            params.pageSize = this.pageMaxNum;
            $.post({
                url: '/marketingActive/activeDataQuery/userInfoSearch/getList.ajax',
                data: params,
                ...
                success: function (result) {
                    if (result.error === 0) {
                        if (result.data.rows) {
                            ...
                            _this.showDialog('', 'info', false, '数据为空');
                            _this.currentIndex = 0;
                            _this.totalPage = 0;
                            _this.tableData = [];
                        }
