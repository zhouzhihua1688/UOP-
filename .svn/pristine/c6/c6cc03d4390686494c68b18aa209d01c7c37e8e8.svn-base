new Vue({
    ...
    methods: {
        ...
        getTableData: function (currentIndex) {
            ...
            $.post({
                url: '/investmentMgmt/investmentStrategy/combinationProductRisk/getTableData.ajax',
                data: params,
        ...
        clearRisk:function(){
            ...
            if(this.isAllUpdate){
                ...
                this.dialogData='';  // 20210106修改，否则saveRisk时候，Object.assign(this.dialogData, ...会报错
            }else{
                this.rightLimit = '';
        ...
        submit:function(item){
            ...
            if(item.fundgroupNewFundgroupDO&&(!item.fundgroupNewFundgroupDO.rightLimit||item.fundgroupNewFundgroupDO.rightLimit=='0')){
                this.showDialog('', 'info', false, '权益类基金定义不能为空');
                return false;
            }
            $.post({
                url: '/investmentMgmt/investmentStrategy/combinationProductRisk/submit.ajax',
                data:params,
                success: function (result) {
                    if (result.error === 0) {
        ...
        saveRisk:function(){
            var _this=this;
            var params={};
            if(!this.rightLimit&&this.rightLimit!='0'){
                this.showDialog('riskDialog', 'info', true, '权益类基金定义不能为空');
                return false;
            }
            ...
            console.log(params);
            $.post({
                url: '/investmentMgmt/investmentStrategy/combinationProductRisk/saveRisk.ajax',
                data: params,
                success: function (result) {
                    if (result.error === 0) {
                        _this.getTableData(0);
                        _this.showDialog('riskDialog', 'info', false, result.msg);
                    }
                    else {
                        _this.showDialog('riskDialog', 'info', true, result.msg);
                    }
                }
            });
        },
