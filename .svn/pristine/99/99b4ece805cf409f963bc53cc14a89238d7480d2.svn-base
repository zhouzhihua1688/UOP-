...
const apiUrl = require('../apiConfig').IPOMgmtFD.IPOMgmtFund;
const tableName = 'bm_ipo_fund';
module.exports = function (app) {
    // 获取  select选择项数据
    app.post('/businessMgmt/IPOMgmtFD/IPOMgmtFund/selectOption.ajax', (req, res, next) => {
        let params = req.body;
        let option = {
            pageUrl: '/businessMgmt/IPOMgmtFD/IPOMgmtFund/selectOption.ajax',
            req,
    ...
    // 获取  基金名称查询数据
    app.post('/businessMgmt/IPOMgmtFD/IPOMgmtFund/collection.ajax', (req, res, next) => {
        let params = req.body;
        let userid = req.session.loginInfo.userid;
        let option = {
            pageUrl: '/businessMgmt/IPOMgmtFD/IPOMgmtFund/collection.ajax',
            req,
            qs: params,
    ...
    // 获取  本地初始数据和查询
    app.post('/businessMgmt/IPOMgmtFD/IPOMgmtFund/getLocalList.ajax', (req, res, next) => {
        let params = req.body;
        let userid = req.session.loginInfo.userid;
        ...
        sql_query(res, $sql, [], function (err, data) { //新增
            if (err) {
                console.log('/businessMgmt/IPOMgmtFD/IPOMgmtFund/getLocalList.ajax', err)
                return res.send({
                    error: 1,
                    msg: '查询失败'
                });
                ...
                let option = {
                    pageUrl: '/businessMgmt/IPOMgmtFD/IPOMgmtFund/getLocalList.ajax',
                    req,
    ...
    // 获取  线上初始数据和查询
    app.post('/businessMgmt/IPOMgmtFD/IPOMgmtFund/getLineList.ajax', (req, res, next) => {
        let params = req.body;
        let userid = req.session.loginInfo.userid;
        let option = {
            pageUrl: '/businessMgmt/IPOMgmtFD/IPOMgmtFund/getLineList.ajax',
            req,
    ...
    // 删除本地数据
    app.post('/businessMgmt/IPOMgmtFD/IPOMgmtFund/deleteFund.ajax', (req, res, next) => {
        let params = req.body;
        pool.getConnection(function (err, connection) {
            if (err) {
                console.log('/businessMgmt/IPOMgmtFD/IPOMgmtFund/deleteFund.ajax  链接失败', err)
                return;
            }
            console.log('params', params)
