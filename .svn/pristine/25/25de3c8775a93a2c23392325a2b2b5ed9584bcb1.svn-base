jQuery.extend({
    ...
    createUploadForm: function(id, fileElementId, data) { //id为当前系统时间字符串，fileElementId为页面<input type='file' />的id，data的值需要根据传入json的键来决定  
        ...
        var newElement = jQuery(oldElement).clone(true).val(''); //克隆页面中的<input type='file' />对象  
    ...
    ajaxFileUpload: function(s) { //这里s是个json对象，传入一些ajax的参数  
        ...
        console.log("ajaxFileUpload s=", s);
    ...
    uploadHttpData: function(r, type) {
        ...
        console.log("uploadHttpData data=", data);
        if (type == "script")
            jQuery.globalEval(data); // Get the JavaScript object, if JSON is used.  
        if (type == "json")
            // data = jQuery(data).text();
            eval("data = " + jQuery(data).text()); // evaluate scripts within html  
            // data = jQuery.parseJSON(jQuery(data).text());
        if (type == "html")
            jQuery("<div>").html(data).evalScripts();
        console.log("uploadHttpData data2=", data);
        return data;
    }
})
