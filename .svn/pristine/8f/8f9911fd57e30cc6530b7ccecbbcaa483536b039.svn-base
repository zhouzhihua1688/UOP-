new Vue({
    ...
    mounted: function () {
        ...
        //查询推荐类型接口
        $.post({
            url: '/recommendSystem/recommendSystemTemplateConfig/homePgButtonConfig/queryRecommendTp.ajax',
        ...
        //获取渠道列表
        $.post({
            url: '/recommendSystem/recommendSystemTemplateConfig/homePgButtonConfig/channelMenu.ajax',
    ...
    methods: {
        ...
        getOneDetail: function (objconfigId) {
            ...
            params.funcBtnId = objconfigId;
            // console.log(params);
            _this.loadingStatus = '数据获取中...';
            $.post({
                url: '/recommendSystem/recommendSystemTemplateConfig/homePgButtonConfig/queryInfo.ajax',
                data: params,
                success: function (result) {
                    if (result.error === 0) {
        ...
        getViewData: function (val,once) {
            ...
            params.contentTp = 'funcbtn';
            val && (params.temId = val);
            $.post({
                url: '/recommendSystem/recommendSystemTemplateConfig/homePgButtonConfig/viewList.ajax',
                data: params,
                success: function (result) {
                    if (result.error === 0) {
                        if (result.data.rows.length > 0) {
        ...
        getTableData: function (currentIndex) {
            ...
            params.pageSize = this.pageMaxNum;
            params.funcBtnName = this.funcBtnName;
        ...
        setAddData: function (obj) {
            this.diafuncBtnName = obj.funcBtnName ? obj.funcBtnName : '';
        ...
        add: function () {
            ...
            var _this = this;
            var params = {};
            params.funcBtnName = this.diafuncBtnName;
            ...
            this.isUpdate && (params.funcBtnId = this.updateId);
            !this.isUpdate && (params.createBy = this.userId);
            this.isUpdate && (params.modifyBy = this.userId);
            var url = '/recommendSystem/recommendSystemTemplateConfig/homePgButtonConfig/';
        ...
        uploadPic1: function () {
            ...
            var _this = this;
            $.ajaxFileUpload({
                url: '/recommendSystem/recommendSystemTemplateConfig/homePgButtonConfig/upload1.ajax',
                type: 'POST',
                dataType: 'json',
        ...
        deleteParams: function (item) {
            this.deleteId = item.funcBtnId;
            this.showDialog('', 'delete1', false);
        },
        deleteConfirm: function () {
            var _this = this;
            $.post({
                url: '/recommendSystem/recommendSystemTemplateConfig/homePgButtonConfig/delete.ajax',
                data: {funcBtnId: this.deleteId},
                success: function (result) {
                    if (result.error === 0) {
        ...
        fresh: function () {
            var _this = this;
            $.post({
                url: '/recommendSystem/recommendSystemTemplateConfig/homePgButtonConfig/fresh.ajax',
                success: function (result) {
                    if (result.error === 0) {
                        _this.showDialog('', 'info', false, '操作成功');
                    } else {
        ...
        configMgmt:function(){
            ...
            for (var j = 0; j < this.tableData.length; j++) {
                if (this.tableData[j].check) {
                    this.tableData[j].objconfigId=this.tableData[j].funcBtnId;
                    this.tableData[j].startTime='';
                    this.tableData[j].endTime='';
                    this.tableData[j].modifyTime='';
                    this.tableData[j].typeKey='funcbtn';
            ...
            if(!window.localStorage){
                ...
                var url="/recommendSystem/recommendSystemConfigMgmt/positionTheThemeMgmt.html?pageType=Modify&contentTp=funcbtn&channelId="+channelIds[0]
                storage.setItem("funcbtn",JSON.stringify(ids));
                window.open(url);
            }
        ...
        configThis:function(item){
            ...
            if(!window.localStorage){
                ...
                var storage=window.localStorage;
                var url="/recommendSystem/recommendSystemConfigMgmt/positionTheThemeMgmt.html?pageType=Modify&contentTp=funcbtn&channelId="+channelIds+'&objconfigId='+ids[0].objconfigId;
                storage.setItem("funcbtn",JSON.stringify(ids));
                window.open(url);
            }
        },
