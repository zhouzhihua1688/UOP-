...
module.exports = function (app) {
    ...
    // 获取材料主题字段的名字
    app.post('/contentMgmt/productMaterial/investPrefecture/getThemeList.ajax', (req, res, next) => {
        // let params = {};
        let params = req.body;
        let option = {
            pageUrl: '/contentMgmt/productMaterial/investPrefecture/getThemeList.ajax',
            req,
    ...
    // 上传封面图片
    app.post('/contentMgmt/productMaterial/investPrefecture/uploadImgCover.ajax', (req, res, next) => {
        try {
            !fs.existsSync(apiConfig.filePathExternal) && fs.mkdirSync(apiConfig.filePathExternal);
            !fs.existsSync(filePathExternal) && fs.mkdirSync(filePathExternal);
            !fs.existsSync(filePathExternalReal) && fs.mkdirSync(filePathExternalReal);
            ...
            console.log('/contentMgmt/productMaterial/investPrefecture/uploadImgCover.ajax -------- 文件上传失败error:', error);
                res.send({
                        error: 0,
                        msg: '上传失败',
                        data: null
