...
module.exports = function (app) {
    // 获取本地数据
    app.post('/systemMgmt/systemLogMgmt/logMgmt/getTableData.ajax', (req, res, next) => {
        try {
            // 获取本地数据库列表
            pool.getConnection(function (error, connection) {
                if (error) {
                    console.log('/systemMgmt/systemLogMgmt/logMgmt/getTableData.ajax 链接本地数据库失败', error.message);
                ...
                console.log('/systemMgmt/systemLogMgmt/logMgmt/getTableData.ajax run SQL:', SQL);
                let getTableData = new Promise((resolve, reject) => {
                    connection.query(SQL, function (error, results) {
                        console.log('/systemMgmt/systemLogMgmt/logMgmt/getTableData.ajax run SQL error', error);
                        console.log('/systemMgmt/systemLogMgmt/logMgmt/getTableData.ajax run SQL results:', Array.from(results).filter(item => (item.operate_type != 0 && item.operate_type != 4)));
                        if (error) {
                ...
                let getTotalCount = new Promise((resolve, reject) => {
                    connection.query(SQL2, function (error, results) {
                        console.log('/systemMgmt/systemLogMgmt/logMgmt/getTableData.ajax run SQL error', error);
                        console.log('/systemMgmt/systemLogMgmt/logMgmt/getTableData.ajax run SQL results:', results);
                        if (error) {
                            reject({error: '运行SQL语句失败'});
                        }
                        resolve(results);
