new Vue({
    ...
    mounted: function () {
        ...
        var _this = this;
        $.post({
            url: '/authorityMgmt/publicConfig/users/getUserInfo.ajax',
    ...
    methods: {
        ...
        showDelete: function () {
            ...
            this.distributedUserViewData.forEach(function (item) {
                ...
                for (var i = 0; i < item.roleList.length; i++) {
                    if (item.roleList[i].name.split('|')[0] == '公共配置') {
        ...
        deleteRole: function () {
            ...
            $.post({
                url: '/authorityMgmt/publicConfig/users/deleteUserRole.ajax',
                data: params,
                success: function (result) {
                    if (result.error == 0) {
                        _this.deleteId = '';
        ...
        checkTree: function (item) {
            ...
            $.post({
                url: '/authorityMgmt/publicConfig/users/checkTree.ajax',
        ...
        updateRole: function (item) {
            ...
            if (this.AllRoleList.length === 0) {
                $.post({
                    url: '/authorityMgmt/publicConfig/users/getRoleInfo.ajax',
                    async: false,
                    success: function (result) {
        ...
        distributeRole: function () {
            ...
            var _this = this;
            $.post({
                url: '/authorityMgmt/publicConfig/users/distributeRole.ajax',
                data: params,
                success: function (result) {
                    if (result.error === 0) {
                        _this.getUserInfo();
