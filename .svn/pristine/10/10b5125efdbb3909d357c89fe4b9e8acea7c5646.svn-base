new Vue({
    ...
    methods: {
        ...
        getOneDetail: function (objconfigId) {
            ...
            params.popupConfigId = objconfigId;
            // params.channelId=this.searchChannelId;
            // console.log(params);
            _this.loadingStatus = '数据获取中...';
            $.post({
                url: '/recommendSystem/recommendSystemTemplateConfig/popUpsConfig/queryInfo.ajax',
                data: params,
        ...
        getViewData: function (val, once) {
            ...
            params.contentTp = 'popup';
            val && (params.temId = val);
            $.post({
                url: '/recommendSystem/recommendSystemTemplateConfig/popUpsConfig/viewList.ajax',
                data: params,
                success: function (result) {
                    if (result.error === 0) {
                        if (result.data.rows.length > 0) {
        ...
        setAddData: function (obj) {
            this.diapopTitle = obj.popTitle ? obj.popTitle : '';
            this.diapopContent = obj.popContent ? obj.popContent : '';
        ...
        showUpdate: function (item) {
            var _this = this;
            this.bufferData=[];
            item.popupButtons&&(this.bufferData=JSON.parse(item.popupButtons))
            ...
            this.updateId = item.popupConfigId;
        ...
        add: function () {
            ...
            var params = {};
            // this.diapopupButtons = obj.popupButtons ? obj.popupButtons : '';
            params.popTitle = this.diapopTitle;
            params.popContent = this.diapopContent;
            params.popupButtons = this.bufferData.length>0?JSON.stringify(this.bufferData):'';
            ...
            this.isUpdate && (params.popupConfigId = this.updateId);
            this.isUpdate && (params.modifyBy = this.userId);
            !this.isUpdate && (params.createBy = this.userId);
            var url = '/recommendSystem/recommendSystemTemplateConfig/popUpsConfig/';
        ...
        deleteParams: function (item) {
            this.deleteId = item.popupConfigId;
            this.showDialog('', 'delete1', false);
        },
        deleteConfirm: function () {
            var _this = this;
            $.post({
                url: '/recommendSystem/recommendSystemTemplateConfig/popUpsConfig/delete.ajax',
                data: {
                    popupConfigId: this.deleteId
                },
                success: function (result) {
                    if (result.error === 0) {
        ...
        fresh: function () {
            var _this = this;
            $.post({
                url: '/recommendSystem/recommendSystemTemplateConfig/popUpsConfig/fresh.ajax',
                success: function (result) {
                    if (result.error === 0) {
                        _this.showDialog('', 'info', false, '操作成功');
                    } else {
        ...
        configMgmt:function(){
            ...
            for (var j = 0; j < this.tableData.length; j++) {
                if (this.tableData[j].check) {
                    this.tableData[j].objconfigId=this.tableData[j].popupConfigId;
                    this.tableData[j].startTime='';
                    this.tableData[j].endTime='';
                    this.tableData[j].modifyTime='';
                    this.tableData[j].typeKey='popup';
            ...
            if(!window.localStorage){
                ...
                var url="/recommendSystem/recommendSystemConfigMgmt/positionTheThemeMgmt.html?pageType=Modify&contentTp=popup&channelId="+channelIds[0]
                storage.setItem("popup",JSON.stringify(ids));
                window.open(url);
            }
        ...
        uploadPic1: function () {
            ...
            var _this = this;
            $.ajaxFileUpload({
                url: '/recommendSystem/recommendSystemTemplateConfig/popUpsConfig/upload1.ajax',
                type: 'POST',
                dataType: 'json',
        ...
        configThis:function(item){
            ...
            var obj={
                objconfigId:item.popupConfigId,
                startTime:'',
                endTime:'',
                modifyTime:'',
                typeKey:'popup',
            ...
            if(!window.localStorage){
                ...
                var storage=window.localStorage;
                var url="/recommendSystem/recommendSystemConfigMgmt/positionTheThemeMgmt.html?pageType=Modify&contentTp=popup&channelId="+channelIds+'&objconfigId='+ids[0].objconfigId;
                storage.setItem("popup",JSON.stringify(ids));
                window.open(url);
            }
        },
