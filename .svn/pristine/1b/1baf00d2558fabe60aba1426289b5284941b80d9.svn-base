new Vue({
    ...
    methods: {
        ...
        getOneDetail: function (objconfigId) {
            ...
            params.noticeId = objconfigId;
            // params.channelId=this.searchChannelId;
            // console.log(params);
            _this.loadingStatus = '数据获取中...';
            $.post({
                url: '/recommendSystem/recommendSystemTemplateConfig/informTemplateConfig/queryInfo.ajax',
                data: params,
        ...
        getViewData: function (val,once) {
            ...
            params.contentTp = 'notice';
            val && (params.temId = val);
            $.post({
                url: '/recommendSystem/recommendSystemTemplateConfig/informTemplateConfig/viewList.ajax',
                data: params,
                success: function (result) {
                    if (result.error === 0) {
                        if (result.data.rows.length > 0) {
        ...
        getTableData: function (currentIndex) {
            ...
            params.pageSize = this.pageMaxNum;
            params.noticeValue = this.noticeValue;
        ...
        setAddData: function (obj) {
            this.dianoticeValue = obj.noticeValue ? obj.noticeValue : '';
        ...
        add: function () {
            ...
            params.noticeValue = this.dianoticeValue;
            ...
            this.isUpdate && (params.noticeId = this.updateId);
            !this.isUpdate && (params.createBy = this.userId);
            this.isUpdate && (params.modifyBy = this.userId);
            var url = '/recommendSystem/recommendSystemTemplateConfig/informTemplateConfig/';
        ...
        uploadPic1: function () {
            ...
            var _this = this;
            $.ajaxFileUpload({
                url: '/recommendSystem/recommendSystemTemplateConfig/informTemplateConfig/upload1.ajax',
                type: 'POST',
                dataType: 'json',
        ...
        deleteConfirm: function () {
            var _this = this;
            $.post({
                url: '/recommendSystem/recommendSystemTemplateConfig/informTemplateConfig/delete.ajax',
                data: {noticeId: _this.deleteId},
                success: function (result) {
                    if (result.error === 0) {
        ...
        fresh: function () {
            var _this = this;
            $.post({
                url: '/recommendSystem/recommendSystemTemplateConfig/informTemplateConfig/fresh.ajax',
                success: function (result) {
                    if (result.error === 0) {
                        _this.showDialog('', 'info', false, '操作成功');
                    } else {
        ...
        configMgmt:function(){
            ...
            for (var j = 0; j < this.tableData.length; j++) {
                if (this.tableData[j].check) {
                    this.tableData[j].objconfigId=this.tableData[j].noticeId;
                    this.tableData[j].startTime='';
                    this.tableData[j].endTime='';
                    this.tableData[j].modifyTime='';
                    this.tableData[j].typeKey='notice';
            ...
            if(!window.localStorage){
                ...
                var url="/recommendSystem/recommendSystemConfigMgmt/positionTheThemeMgmt.html?pageType=Modify&contentTp=notice&channelId="+channelIds[0]
                storage.setItem("notice",JSON.stringify(ids));
                window.open(url);
            }
        ...
        configThis:function(item){
            ...
            if(!window.localStorage){
                ...
                var storage=window.localStorage;
                var url="/recommendSystem/recommendSystemConfigMgmt/positionTheThemeMgmt.html?pageType=Modify&contentTp=notice&channelId="+channelIds+'&objconfigId='+ids[0].objconfigId;
                storage.setItem("notice",JSON.stringify(ids));
                window.open(url);
            }
        },
