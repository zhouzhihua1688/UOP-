new Vue({
    ...
    methods: {
        ...
        getTableData: function (currentIndex) {
            ...
            $.post({
                url: '/messageCenter/auditMgmt/headToReview/getList.ajax',
                data: params,
        ...
        passCheck:function (val) {
            ...
            $.post({
                url: '/messageCenter/auditMgmt/headToReview/passCheck.ajax',
                data: params,
                success: function (result) {
                    if (result.error === 0) {
                        var $index=_this.currentIndex;
                        // console.log(result,'open');
                        _this.showDialog('', 'info', false, '头像审核通过');
        ...
        rejectChecking:function () {
            ...
            $.post({
                url: '/messageCenter/auditMgmt/headToReview/rejectCheck.ajax',
                data: params,
                success: function (result) {
                    if (result.error === 0) {
                        var $index=_this.currentIndex;
