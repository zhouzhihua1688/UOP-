...
var vm = new Vue({
    ...
    watch: {
        ...
        threeClassifyId() {
            console.log('this.threeClassifyId=', this.threeClassifyId)
            // 海报：220＊391，
            // 视频：690＊388
            // 其他：200*200
            let mappingImgObj =[
            ...
            if(this.threeClassifyId=='C000239'){  //基金经理视频
    ...
    mounted: function () {
        var dialogs = ['info', 'del','setClassify'];
        ...
        $('.select2').css('width','150px').select2({closeOnSelect: false});
        ...
        // 获取渠道
        $.post({
            url:'/contentMgmt/operationMaterial/articleResource/getbranchList.ajax',
    ...
    methods: {
        getTagAll:function(){
           ...
           var _this=this;
            $.post({
                url:'/contentMgmt/operationMaterial/articleResource/getTagAll.ajax',
                data:{
                    parentCategoryId:''
                },
                success: function (result) {
    ...
    // 获取基金列表
        fundList:function(){
            var _this = this;
            $.post({
                url: '/contentMgmt/operationMaterial/articleResource/fundList.ajax',
                success: function (result) {
                    if (result.error === 0) {
...
// 获取基金经理列表 
        getManagerList: function() {
            var _this = this;
            $.post({
                url: '/contentMgmt/operationMaterial/articleResource/getManagerList.ajax',
                success: function(result) {
                    if (result.error === 0) {
 ...
 // 主页面数据
        ...
        getTableData: function (currentIndex) {
            ...
            _this.tableData=[];
            $.post({
                url: '/contentMgmt/operationMaterial/articleResource/getTableData.ajax',
                data: params,
                success: function (result) {
                    $(".cover").hide();
                    if (result.error === 0) {
                        ...
                        if(result.data.body && Array.isArray(result.data.body.mediaUopVos) ) {
                             // 用 MediaId获取材料主题
                            let getMediaId=result.data.body.mediaUopVos.map(function(item){
                                return item.mediaId;
                             })
                            if(getMediaId.length>0){
                                _this.getThemeList(getMediaId.toString());  //获取对应的资源类别
                            }
                            _this.tableData = result.data.body.mediaUopVos&&params.categoryId?result.data.body.mediaUopVos:[];
                        }else{
                            _this.showText='暂无数据';
                        }
                    }
         ...
         // 主页面:根据mediaIdList查询parentCategoryId下的素材分类，资源类别
        getThemeList:function(mediaIdList){
            ...
            console.log(params);
            $.post({
                    url: '/contentMgmt/operationMaterial/articleResource/getThemeList.ajax',
                    data: params,
                    success: function (result) {
                        if (result.error === 0) {
        ...
        //   params.branchCodeList =['00'];  //目前写死--渠道码 ,
          params.branchCodeList =_this.sourceTypeChannel.split();  //渠道权限 ,
        ...
        //   params.publishTime="20220421"; 
          ...
          let url='';
          if(!_this.updateId){
             url='/contentMgmt/operationMaterial/articleResource/add.ajax'
          }else{
              params.mediaId=_this.updateId;
             url='/contentMgmt/operationMaterial/articleResource/update.ajax'
          }
            $.post({
                url:url,
                data:params,
...
// start-现有材料相关操作--弹窗
        ...
        chooseMaterialId:function(materialId,num){
            ...
            console.log(params);
            $.post({
                url: '/contentMgmt/operationMaterial/articleResource/threeClassifyList.ajax',
                data: params,
                success: function (result) {
                    if (result.error === 0) {
                        console.log(result)
        ...
        searchMaterial:function(currentIndex){
            var _this=this;
                ...
                $.post({
                    url: '/contentMgmt/operationMaterial/articleResource/searchMaterial.ajax',
                    data: params,
                    success: function (result) {
                        if (result.error === 0) {
                            if(currentIndex){
        ...
        //    _this.categoryIdList.push(_this.firstClassifyId,_this.secondClassifyId,_this.threeClassifyId);
            ...
            console.log(params);
            $.post({
                url: '/contentMgmt/operationMaterial/articleResource/getUpdateDetails.ajax',
                data: params,
                success: function (result) {
                    if (result.error === 0) {
        ...
        //    _this.option.img = item.displayImageUrl;  //显示封面图片-裁剪
            ...
            $.post({
                ...
                success: function (result) {
                    if (result.error === 0) {
                        ...
                        _this.mediaId=result.data.body[0].mediaId;                      //只有修改当前的mediaId不会变
                        ...
                        if(_this.mediaType=='link'){    //file text image  linkWW
                           _this.url=result.data.body[0].url;
                        }
                        ...
                        // _this.updateTime=result.data.body[0].publishTime?result.data.body[0].publishTime:'';
                        // _this.updateTime=_this.updateTime?moment(_this.updateTime).format('YYYY-MM-DD HH:mm:ss'):'';
                        // if(_this.pushStatus=='1'){   //当为上架状态时候
                        //     _this.publishTime=_this.updateTime;
                        // }
                        _this.updateTime=result.data.body[0].modifyTime?result.data.body[0].modifyTime.replace(/(\d{4})(\d{2})(\d{2})(\d{2})(\d{2})(\d{2})/g, '$1-$2-$3 $4:$5:$6'):'';
                        _this.publishTime=result.data.body[0].publishTime?result.data.body[0].publishTime.replace(/(\d{4})(\d{2})(\d{2})(\d{2})(\d{2})(\d{2})/g, '$1-$2-$3 $4:$5:$6'):''; 

                        // _this.option.img = result.data.body[0].displayImageUrl;  //显示封面图片-裁剪
                        // _this.option.img ='http://10.50.115.48/uopStatic/contentMgmt/productMaterial/investPrefecture/%E9%85%8D%E5%9B%BE160x160.png'
                        _this.displayImageUrl = result.data.body[0].displayImageUrl;//显示封面图片
...
//start-删除数据
        ...
        del:function(){
            ...
            console.log(params);
            $.post({
                url: '/contentMgmt/operationMaterial/articleResource/del.ajax',
                data: params,
                success: function (result) {
                    if (result.error === 0) {
                        setTimeout(function(){
...
//end-删除数据
    ...
    //    一级标签
        getTagList:function(){
            ...
            var params={}
            $.post({
                url:'/contentMgmt/operationMaterial/articleResource/getTagList.ajax',
                // data:params,
                success: function (result) {
                    if (result.error === 0) {
        ...
        getSecondTagList:function(tagCategoryId,i){
            ...
            console.log(params);
            $.post({
                url:'/contentMgmt/operationMaterial/articleResource/getSecondTagList.ajax',
                data:params,
                success: function (result) {
                    if (result.error === 0) {
                        console.log(result);
        ...
        setClassify:function(){
          ...
          var hasCheck = false;
            ...
            _this.showDialog('', 'setClassify');
        },
 
    // 确定设置分类
        checkSetClassify:function(){
            var _this=this;
            var params={}
              // 获取选中的mediaIdList
            var getMediaIds= [];
            for (var i = 0; i < this.tableData.length; i++) {
                if (this.tableData[i].check) {
                    getMediaIds.push(this.tableData[i].mediaId);
                }
            }

            // _this.getCategoryIdList(getMediaIds.toString()); //用mediaIdList查询该数据的categoryIdList==三级分类
            
            // $.post({
            //     url: '/contentMgmt/operationMaterial/articleResource/getCategoryIdList.ajax',
            //     data: params,
            //     success: function (result) {
            //         if (result.error === 0) {
            //             let arryList=[];
            //             result.data.body.forEach(function(item){
            //                 arryList=arryList.concat(item.categoryList.slice(-1));  //获取每个数组的最后一个分类，然后合并
            //             }) 
            ...
            if (!_this.setClassifySingle) {
                this.showDialog('setClassify', 'info', true, '请选择分类');
                return false;
            }
            params.mediaIdList= getMediaIds;
            params.parentCategoryId=_this.secondClassifyId;
            params.targetCategoryIdList=[_this.setClassifySingle];
            console.log(params);
            $.post({
                url:'/contentMgmt/operationMaterial/articleResource/checkSetClassify.ajax',
                data:params,
                success: function (result) {
                    if (result.error === 0) {
                        // console.log(result);
                        _this.getTableData(0);
                        _this.showDialog('setClassify', 'info', false,result.msg);
                    }else {
                        _this.showDialog('setClassify', 'info', true,result.msg);
                    }
                }
            })
        ...
        uploadsFile(e) {
            ...
            $(".cover").show();
            $.post({
                url: '/contentMgmt/operationMaterial/articleResource/uploadFile.ajax',
                type: 'post',
                dataType: 'json',
                data: formdata,
                processData: false,
        ...
        uploadImg:function(e){
            ...
            var reader = new FileReader()
                ...
                reader.onload = (e) => {
                    ...
                    $.post({
                        url: '/contentMgmt/operationMaterial/articleResource/uploadImg.ajax',
		...
		uploadForServer() {
			...
			var _this = this;
				...
				$.post({
					url: '/contentMgmt/operationMaterial/articleResource/uploadImgCover.ajax',
					// data: res.compressBase64,
					cache: false,
					data: formdata,
					processData: false,
