new Vue({
    ...
    methods: {
        ...
        getTableData: function (currentIndex) {
            ...
            $.post({
                url: '/recommendSystem/recommendSystemConfigMgmt/groupRelationTypeConfig/queryInfo.ajax',
                data: params,
        ...
        setAddData: function (obj) {
            this.diatypeKey = obj.typeKey ? obj.typeKey : '';
            this.diatypeName = obj.typeName ? obj.typeName : '';
            this.diatypeLevel = obj.typeLevel ? obj.typeLevel : '';
        },
        showAdd: function () {
            this.isUpdate = false;
            this.setAddData({});
        ...
        add: function () {
            var _this = this;
            if(this.diatypeKey==''||this.diatypeName==''){
                _this.showDialog('add', 'info', true, '类型ID，名称必须填写！');
                return;
            }
            var params = {};
            params.typeKey = this.diatypeKey;
            params.typeName = this.diatypeName;
            params.typeLevel = this.diatypeLevel;
            !this.isUpdate && (params.createBy = this.userId);
            var url = '/recommendSystem/recommendSystemConfigMgmt/groupRelationTypeConfig/';
        ...
        deleteParams: function (item) {
            this.deleteId=item.typeKey;
            this.showDialog('', 'delete1', false);
        },
        deleteConfirm:function () {
            var _this = this;
            $.post({
                url: '/recommendSystem/recommendSystemConfigMgmt/groupRelationTypeConfig/delete.ajax',
                data: {typeKey: this.deleteId},
                success: function (result) {
                    if (result.error === 0) {
