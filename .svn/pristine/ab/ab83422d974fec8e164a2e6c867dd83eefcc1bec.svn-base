...
var vm = new Vue({
    ...
    methods: {
        ...
        getTableData: function (currentIndex) {
            ...
            console.log(params);
            $.post({
                url: '/contentMgmt/tagMgmt/tagClassifyMgmt/getTableData.ajax',
                data: params,
        ...
        diaInfoCheck: function () {
            ...
            //     return false;
            // }
            if (!this.tagCategoryName) {
                this.showDialog('add', 'info', true, '标签分类名称不能为空');
                return false;
            }
            return true;
        },
        add:function(){
            ...
            let url='';
            if(!this.updateId){
               url='/contentMgmt/tagMgmt/tagClassifyMgmt/add.ajax'
            }else{
               url='/contentMgmt/tagMgmt/tagClassifyMgmt/update.ajax'
            }
            if (this.diaInfoCheck()) {
                var params={}
                params.tagCategoryId=_this.tagCategoryId;
                params.tagCategoryName=_this.tagCategoryName;
                $.post({
                    url:url,
                    data:params,
                    success: function (result) {
         ...
         showUpdate:function (item) {
            ...
            _this.updateId=item.tagCategoryId;
            _this.tagCategoryId=item.tagCategoryId;
            _this.tagCategoryName=item.tagCategoryName;
            this.showDialog('', 'add');
        },
             // 删除
        ...
        del:function() {
                ...
                console.log(params);
                $.post({
                    url: '/contentMgmt/tagMgmt/tagClassifyMgmt/del.ajax',
                    data: params,
                    success: function(result) {
                        if (result.error === 0) {
                            _this.getTableData(0);
