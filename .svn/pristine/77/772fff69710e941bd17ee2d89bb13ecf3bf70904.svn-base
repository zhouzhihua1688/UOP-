var vm = new Vue({
	...
	methods: {
		getList: function() {
			...
			//体验金参数list
			$.post({
				url: '/awardMgmt/experienceGold/rewardSetting/configList.ajax',
				data: params,
				success: function (result) {
					if (result.error === 0) {
						if(result.data.rows.length>0){
							result.data.rows.forEach(function(item){
								_this.selectForNodeIds.push({"pkey":item.pkey,"pname":item.pname})
							})
						}
					}
					else {
						_this.selectForNodeIds = [];
						_this.showDialog('', 'info', false, result.msg);
					}
				}
			});
		...
		showUpdate: function(item) {
			...
			this.dia_rewardDetail = conditionJson.rewardType == '01'?conditionJson.rewardDetail.split('|')[0]:conditionJson.rewardDetail;
			this.dia_rewardDetailNumber = conditionJson.rewardType == '01'?conditionJson.rewardDetail.split('|')[1]:'';			
			this.dia_logicFlag = true;
			this.dia_id = item.id;
		...
		addSave: function() {
			...
			params.subapkind = this.dia_subapkind;
		...
		deleteData: function() {
			...
			$.post({
				url: '/awardMgmt/experienceGold/rewardSetting/del.ajax',
				data: params,
				success: function(result) {
					if (result.error === 0) {
						_this.getTableData(0);
