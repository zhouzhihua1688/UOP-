new Vue({
    ...
    methods: {
        ...
        search: function (id) {
            ...
            $.post({
                url: '/publicConfig/appointment/appointmentMgmt/queryListForDetail.ajax',
                data: params,
        ...
        getDialogData: function () {
            $.post({
                url: '/publicConfig/appointment/appointmentMgmt/getDialogDataForDetail.ajax',
                success: function (result) {
                    if (result.error === 0) {
        ...
        showConfirmTime: function (item) {
            this.id = item.id;
            this.preorderActId = item.preorderActId;
            this.custNo = item.custNo;
            this.confirmTakepartTtime = item.confirmTakepartTtime ? item.confirmTakepartTtime.slice(0, -2) : '';
            $('.msgRuleList').val('');
            $('.msgRuleList').trigger('chosen:updated');
            this.showDialog('', 'confirmTime');
        },
        confirmTime: function () {
            if (!this.confirmTakepartTtime) {
                this.showDialog('confirmTime', 'info', true, '未填写确认时间');
                return;
            }
            if (!this.msgRuleId) {
                this.showDialog('confirmTime', 'info', true, '未选择消息规则');
                return;
            }
            var params = {
                id: this.id,
                actId: this.preorderActId,
                custNo: this.custNo,
                confirmTakepartTtime: this.confirmTakepartTtime,
                messageConfigId: this.msgRuleId
            };
            $.post({
                url: '/publicConfig/appointment/appointmentMgmt/confirmTakepartTtimeForDetail.ajax',
                data: params,
                success: function (result) {
                    if (result.error === 0) {
                        this.search();
                        this.showDialog('confirmTime', 'info', false, '确认消息成功');
                    } else {
                        this.search();
                        this.showDialog('confirmTime', 'info', false, result.msg);
                    }
                }.bind(this)
            });
        },
