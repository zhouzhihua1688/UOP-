new Vue({
    ...
    mounted: function () {
        ...
        var _this = this;
        $.post({
            url: '/authorityMgmt/thirdPartyOperation/users/getUserInfo.ajax',
    ...
    methods: {
        ...
        showDelete: function () {
            ...
            this.distributedUserViewData.forEach(function (item) {
                ...
                for (var i = 0; i < item.roleList.length; i++) {
                    if (item.roleList[i].name.split('|')[0] == '三方运营') {
                        _this.deleteChannel = item.roleList[i].channel;
                    }
                }
            });
            if (userIds.length === 0) {
        ...
        deleteRole: function () {
            ...
            params.appId = this.deleteChannel;
            $.post({
                url: '/authorityMgmt/thirdPartyOperation/users/deleteUserRole.ajax',
                data: params,
                success: function (result) {
                    if (result.error == 0) {
                        _this.deleteId = '';
        ...
        checkTree: function (item) {
            ...
            $.post({
                url: '/authorityMgmt/thirdPartyOperation/users/checkTree.ajax',
        ...
        updateRole: function (item) {
            ...
            if (this.AllRoleList.length === 0) {
                $.post({
                    url: '/authorityMgmt/thirdPartyOperation/users/getRoleInfo.ajax',
                    async: false,
                    success: function (result) {
            ...
            item.roleList.forEach(function (roleInfo) {
                if (roleInfo.name.split('|')[0] == '三方运营') {
                    _this.selectedRoleList.push(roleInfo);
                }
                else {
                    _this.cannotDeleteIds.push(roleInfo.id);
                }
            });
            this.showDialog('', 'add');
        },
        ...
        checkAllRoleTree: function () {
            ...
            if (this.selectedRoleList.length !== 0) {
                this.selectedRoleList.forEach(function (item) {
                    item.menuList = typeof item.menuList == 'string' ? JSON.parse(item.menuList) : item.menuList;
        ...
        distributeRole: function () {
            ...
            var channel = this.selectedRoleList[0].channel;
            this.selectedRoleList.forEach(function (item) {
                roleIds.push(item.id);
            });
            var _this = this;
            roleIds = roleIds.concat(this.cannotDeleteIds);
            var params = {
                channel: channel,
            ...
            var _this = this;
            $.post({
                url: '/authorityMgmt/thirdPartyOperation/users/distributeRole.ajax',
                data: params,
                success: function (result) {
                    if (result.error === 0) {
                        _this.getUserInfo();
