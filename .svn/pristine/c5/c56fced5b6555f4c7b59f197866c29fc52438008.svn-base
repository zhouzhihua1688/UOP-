...
var vm = new Vue({
    ...
    methods: {
        ...
        add: function () {
            ...
            params.shareCoverImage = _this.shareCoverImage;
        ...
        modify: function (item) {
            ...
            _this.option.img = '';  //显示图片
            _this.shareCoverImage='';
            // 查询单个产品信息
            $.post({
        ...
        delFile(){
           ...
           //删除图片按钮置空图片和预览
            ...
            // var nextName = _this.shareCoverImage.split('/')[_this.shareCoverImage.split('/').length - 1];
            // params.fileUrl= nextName;
            
            // 暂时不用掉接口
            // params.fileUrl= _this.shareCoverImage;
            // console.log(params);
            // $.post({
            //     url: '/advertising/productMaterialMgmt/productList/delFile.ajax',
            //     data: params,
            //     success: function (result) {
            //         if (result.error === 0) {
            //             _this.option.img=''; 
