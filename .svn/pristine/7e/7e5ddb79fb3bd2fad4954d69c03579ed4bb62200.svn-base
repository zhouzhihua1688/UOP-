new Vue({
    ...
    mounted: function () {
        ...
        $.post({
            url: '/authorityMgmt/publicConfig/roles/roleData.ajax',
    ...
    methods: {
        check: function (index) {
            ...
            $.post({
                url: '/authorityMgmt/publicConfig/roles/checkTree.ajax',
                data: params,
                success: function (result) {
                    if (result.error === 0) {
                        $('#checkTree').treeview({
        ...
        deleteRole: function () {
            ...
            var _this = this;
            $.post({
                url: '/authorityMgmt/publicConfig/roles/deleteRole.ajax',
                data: params,
                success: function (result) {
                    if (result.error === 0) {
                        _this.deleteID = '';
        ...
        saveRole: function () {
            ...
            var params = {
                name: '公共配置|' + this.diaRoleName,
                menuList: JSON.stringify(menuList)
            };
            this.updateId && (params.id = this.updateId);
            $.post({
                url: '/authorityMgmt/publicConfig/roles/saveRole.ajax',
                data: params,
                success: function (result) {
                    if (result.error === 0) {
                        $.post({
