new Vue({
    ...
    created: function () {
        $.post({ //展示类别
            url: '/businessMgmt/IPOMgmtOCReview/IPOLimit/showChinese.ajax',
            data: {
                pmst: 'SYSTEM',
                pmkey: 'DISPLAYFUNDTP'
            },
    ...
    computed: {
        ...
        viewData: function () {
            ...
            if (this.reviewSt === 'N') {
                return this.middleData[currentIndex].filter(function (item) {
                    if (Object.keys(item.approve).length !== 0) {
                        return true
                    }
                });
            }
    ...
    methods: {
        ...
        getFundIdList: function () {
            var _this = this;
            $.post({
                url: '/businessMgmt/IPOMgmtOCReview/IPOLimit/fundIdList.ajax ',
                success: function (result) {
                    if (result.error == 0) {
        ...
        getTableData: function () {
            ...
            $.post({
                url: '/businessMgmt/IPOMgmtOCReview/IPOLimit/tableData.ajax ',
                data: params,
                success: function (result) {
                    this.reviewSt = this.approveStatus
        ...
        review: function () {
            ...
            $.post({
                url: '/businessMgmt/IPOMgmtOCReview/IPOLimit/review.ajax ',
                data: params,
                success: function (result) {
                    if (result.error == 0) {
                        this.getTableData()
        ...
        windowsGoTo: function (item) {
            window.location.href = '/businessMgmt/IPOMgmtOCReview/IPOLimit.html?pageType=Detail&fundId=' + item.approve.fundid + '&fundStatus=' + item.approve.apprStatus
