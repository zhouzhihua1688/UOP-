...
new Vue({
    ...
    created: function () {
        var _this = this;
        $.post({//基金名称查询
            url: '/businessMgmt/IPOMgmtEC/IPOMgmt/collection.ajax',
            data: {
        ...
        $.post({//注册登记代码
            url: '/businessMgmt/IPOMgmtEC/IPOMgmt/selectOption.ajax',
            data: {
                pmst: 'SYSTEM',
                pmkey: 'TANO'
            },
    ...
    methods: {
        getTableData: function (currentIndex, load) {
            ...
            if (this.dataFlag == 'line') {
                ...
                url = '/businessMgmt/IPOMgmtEC/IPOMgmt/getLineList.ajax';
            } else {
                url = '/businessMgmt/IPOMgmtEC/IPOMgmt/getLocalList.ajax';
        ...
        windowGoTo: function (url, fund) {
            ...
            if (url === 'Detail') {
                window.location.href = '/businessMgmt/IPOMgmtEC/IPOMgmt.html?pageType=Detail&service_id=' + service_id + '&local_id=' + fund.local_id;
                return;
            }
            if (this.dataFlag == 'local') {
                window.location.href = '/businessMgmt/IPOMgmtEC/IPOMgmt.html?pageType=' + url + '&address=' + this.dataFlag + '&service_id=' + service_id + '&local_id=' + fund.local_id;
            } else {
                window.location.href = '/businessMgmt/IPOMgmtEC/IPOMgmt.html?pageType=' + url + '&address=' + this.dataFlag + '&service_id=' + service_id;
            }
        },
        ...
        deleteFund: function () {
            ...
            $.ajax({
                url: '/businessMgmt/IPOMgmtEC/IPOMgmt/deleteFund.ajax',
                type: 'POST',
                data: params,
                success: function (result) {
                    if (result.error == 0) {
