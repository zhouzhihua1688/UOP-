const vm=new Vue({
    ...
    data: {
        ...
        branchCode: [],           //网点-多选
        acceptMode:"",           //渠道
        acceptModeList:[{'value': "0", "name": "柜台"}, {'value': "2", "name": "网上"}, {
            'value': "6",
            "name": "第三方"
        }, {'value': "7", "name": "企业版"}],
    ...
    mounted: function () {
        var dialogs = ['info', 'del', 'del2',"update", "revise", 'subMit', 'delAgain', "addFund", 'addFundHistory', 'muchFundHistory', "checkDatils","wareHouse"];
        var _this = this;
        ...
        $('#acceptMode').multiselect({
            ...
            onChange: function(option, checked, select) {
                if($(option).val()==='6'){
                    $("#acceptMode").next().find('li').each(function(){
                ...
                _this.acceptMode = $("#acceptMode").val() ? $("#acceptMode").val().join(',') : '';
                if( _this.acceptMode.includes('6')){
                    _this.isCode = true;
                }else{
                    _this.isCode = false;
                }
        ...
        $('#acceptModes').multiselect({
            ...
            onChange: function(option, checked, select) {
                // console.log('这是人干的事儿？');
                if($(option).val()==='6'){
                    $("#acceptModes").next().find('li').each(function(){
                        if($(this).find('input').val()!='6'){
                            $(this).find('input').attr('disabled',true)
                        }
                    })
                ...
                _this.acceptMode = $("#acceptModes").val() ? $("#acceptModes").val().join(','):'';
                if( _this.acceptMode.includes('6')){
                    _this.isCode = true;
                }else{
                    _this.isCode = false;
        ...
        $('#acceptModes_serve').multiselect({
            ...
            onChange: function(option, checked, select) {
                if($(option).val()==='6'){
                    $("#acceptModes_serve").next().find('li').each(function(){
                        if($(this).find('input').val()!='6'){
                            $(this).find('input').attr('disabled',true)
                        }
                    })
                ...
                if(!$("#acceptModes_serve").val()){
                    $("#acceptModes_serve").next().find('li').each(function(){
                        $(this).find('input').attr('disabled',false);
                    })
                }
                _this.service.acceptMode = $("#acceptModes_serve").val() ? $("#acceptModes_serve").val().join(','):'';
                // if( _this.acceptMode.includes('6')){
                //     _this.isCode = true;
                // }else{
                //     _this.isCode = false;
    ...
    methods: {
        ...
        allGroupId: function () {
            var _this = this
            $.post({
                url: '/businessMgmt/combinationProductConfig/combinationProductHandle/allGroupId.ajax',
                success: function (result) {
                    if (result.error === 0) {
                ...
                $.post({
                    ...
                    success: function (result) {
                        if (result.error === 0) {
                            ...
                            // //渠道多选条件
                            // _this.dataSummary(_this.acceptModeList, 'value', 'label', 'acceptMode',"");
                            // _this.dataSummary(_this.acceptModeList, 'value', 'label', 'acceptModes',"");
                        }
                        else {
                            _this.tableData=[];
                            _this.loadingStatus = '暂无数据';
        ...
        showAdd: function () {
            ...
            this.riskControl = 'N';////2020.09.28 投顾服务费率
            ...
            _this.groupFundType();
            // 清空新增
            this.dataSummary(this.acceptModeList, 'value', 'label', 'acceptMode',"");
            this.showDialog('', 'add');
        },
        // 同步时间(调仓成立时间)
        syncTime:function(){
        ...
        saveParam: function () {
            ...
            if (this.diaInfoCheck()) {
                ...
                params.fundgroupNewFundgroupDO = {
                    ...
                    rightLimit: this.isInvestment === 'Y' ? this.rightLimit : '',        //权益类基金定义(属组合成分里)
                    largeRedemptionPercent: this.isInvestment === 'Y' ?  this.largeRedemptionPercent : '',        //大额赎回在全平台的比例(属组合成分里)
                    ...
                    rightMaxratePerc:this.isInvestment === 'Y' ?  this.rightMaxratePerc : '',        //权益类基金占比不超过
                    turnoverRatePerc:this.isInvestment === 'Y' ?  this.turnoverRatePerc : '',        //账户换手率
                    singlevalueCustmaxPerc:this.isInvestment === 'Y' ?  this.singlevalueCustmaxPerc : '', // 单只基金市值不得超过客户账户资产净值
                    categoryunitGroupmaxPerc:this.isInvestment === 'Y' ?  this.categoryunitGroupmaxPerc : '',  // 同策略持有基金份额不得超过该基金总份额占比
                    singleunitGroupmaxPerc:this.isInvestment === 'Y' ?  this.singleunitGroupmaxPerc : '',   // 持有单只指数基金的份额总和不得超过该基金总份额的
                    investRiskLevel:this.isInvestment === 'Y' ?  this.investRiskLevel : '',     //投顾风险等级
                    investType:this.isInvestment === 'Y' ?  this.investType : '',       //投顾类型
                    investCustomers:this.isInvestment === 'Y'&&this.investType==='G' ?  this.investCustomers : '',       //投顾组合客户
                    investPrincipal:this.isInvestment === 'Y'&&this.investType==='G' ?  this.investPrincipal : '',       //名义本金
                    investDuration:this.isInvestment === 'Y'&&this.investType==='G' ?  this.investDuration : '',       //投资时长

                    categoryDescDoc:this.isInvestment === 'Y' ?  this.categoryDescDoc : '',        //策略说明书
                    riskDescDoc:this.isInvestment === 'Y' ?  this.riskDescDoc : '',        //风险揭示书
                    investDescDoc:this.isInvestment === 'Y' ?  this.investDescDoc : '',        //投顾协议
                    categoryDescDisplay:this.isInvestment === 'Y' ?  this.categoryDescDisplay : '', //策略说明书文案
                    riskDescDisplay:this.isInvestment === 'Y' ?  this.riskDescDisplay : '',        //风险揭示书文案
                    investDescDisplay:this.isInvestment === 'Y' ?  this.investDescDisplay : '',        //投顾协议文案
        ...
        serviceDetail: function (groupId) {
            ...
            params.type = 0;
            $.post({
                url: '/businessMgmt/combinationProductConfig/combinationProductHandle/details.ajax',
                data: params,
                success: function (result) {
                    if (result.error === 0) {
        ...
        localUpdate: function (item) {
            ...
            if (this.operate == "1") {
                ...
                _this.dataSummary(this.acceptModeList, 'value', 'label', 'acceptMode', this.acceptMode);
                ...
                this.rightMaxratePerc=this.isInvestment === 'Y' ?  item.fundgroupNewFundgroupDO.rightMaxratePerc : '';        //权益类基金占比不超过
                this.turnoverRatePerc=this.isInvestment === 'Y' ?  item.fundgroupNewFundgroupDO.turnoverRatePerc : '';        //账户换手率
                this.singlevalueCustmaxPerc=this.isInvestment === 'Y' ?  item.fundgroupNewFundgroupDO.singlevalueCustmaxPerc : ''; // 单只基金市值不得超过客户账户资产净值
                this.categoryunitGroupmaxPerc=this.isInvestment === 'Y' ?  item.fundgroupNewFundgroupDO.categoryunitGroupmaxPerc : '';  // 同策略持有基金份额不得超过该基金总份额占比
                this.singleunitGroupmaxPerc=this.isInvestment === 'Y' ?  item.fundgroupNewFundgroupDO.singleunitGroupmaxPerc : '';   // 持有单只指数基金的份额总和不得超过该基金总份额的
                this.investRiskLevel=this.isInvestment === 'Y' ?  item.fundgroupNewFundgroupDO.investRiskLevel : '';     //投顾风险等级

                this.investType=this.isInvestment === 'Y' ?  item.fundgroupNewFundgroupDO.investType : '';       //投顾类型
                this.investCustomers=this.isInvestment === 'Y'&&this.investType==='G' ?  item.fundgroupNewFundgroupDO.investCustomers : '';       //投顾组合客户
                this.investPrincipal=this.isInvestment === 'Y'&&this.investType==='G' ?  item.fundgroupNewFundgroupDO.investPrincipal : '';       //名义本金
                this.investDuration=this.isInvestment === 'Y'&&this.investType==='G' ?  item.fundgroupNewFundgroupDO.investDuration : '';       //投资时长

                this.categoryDescDoc=this.isInvestment === 'Y' ?  item.fundgroupNewFundgroupDO.categoryDescDoc : '';        //策略说明书
                this.riskDescDoc=this.isInvestment === 'Y' ?  item.fundgroupNewFundgroupDO.riskDescDoc : '';        //风险揭示书
                this.investDescDoc=this.isInvestment === 'Y' ?  item.fundgroupNewFundgroupDO.investDescDoc : '';        //投顾协议

                this.categoryDescDisplay=this.isInvestment === 'Y' ?  item.fundgroupNewFundgroupDO.categoryDescDisplay : ''; //策略说明书文案
                this.riskDescDisplay=this.isInvestment === 'Y' ?  item.fundgroupNewFundgroupDO.riskDescDisplay : '';        //风险揭示书文案
                this.investDescDisplay=this.isInvestment === 'Y' ?  item.fundgroupNewFundgroupDO.investDescDisplay : '';        //投顾协议文案
            ...
            if (this.operate == "2") {
                ...
                this.acceptMode = item.fundgroupSubdatetimeRO.acceptMode;
                //渠道多选条件
                _this.dataSummary(_this.acceptModeList, 'value', 'label', 'acceptModes', this.acceptMode);
        ...
        localBasic: function () {                                 //只针对修改基本信息
            ...
            if (this.reviseCheck()) {
                ...
                $.post({
                    url: '/businessMgmt/combinationProductConfig/combinationProductHandle/localBasic.ajax',
                    data: params,
                    success: function (result) {
                        if (result.error === 0) {
                            _this.getTableData(0, params.type);
        ...
        houseUpdate: function (item) {
            ...
            $.post({
                url: '/businessMgmt/combinationProductConfig/combinationProductHandle/details.ajax',   //单个基金信息
                data: {
            ...
            //
            $.post({
                url: '/businessMgmt/combinationProductConfig/combinationProductHandle/basicParam.ajax',
                data: params,
                success: function (result) {
                    if (result.error === 0) {
        ...
        serviceUpdate: function (item, index) {
            ...
            // this.acceptMode=this.acceptMode.join(",");
        ...
        localHouse: function () {
            ...
            // $.post({
            //     url: '/businessMgmt/combinationProductConfig/combinationProductHandle/localHouse.ajax',
            //     data: params,
            //     success: function (result) {
            //         if (result.error === 0) {
            //             _this.getTableData(0, params.type);
        ...
        showSubmitAgain: function (item) {
            ...
            // 渠道多选条件
            _this.dataSummary(_this.acceptModeList, 'value', 'label', 'acceptMode', this.acceptMode);
            _this.dataSummary2(_this.branchCodeList, 'value', 'label', 'branchCodes',this.branchCode);
        },
        ...
        dataSummary: function (asynData, value, label, dom,valueCode) {
            ...
            if (asynData && asynData.length > 0) {
                ...
                $("#" + dom).multiselect('dataprovider', data);
                // 下面操作是为了调用onchange事件
                $("#" + dom).multiselect('select',realValue,true);    
            }
        },
        ...
        fundAllIdList:function(itemFundIdList,source,channelList,num,index){
            ...
            console.log(params)
            $.post({
                url: '/businessMgmt/combinationProductConfig/combinationProductHandle/fundIdLists.ajax',
                data:params,
                success: function (result) {
                    if (result.error === 0) {
        ...
        checkNav: function (ids,source,channelList,fundgroupType,index) {
            ...
            $.post({
                url: '/businessMgmt/combinationProductConfig/combinationProductHandle/navList.ajax',
                data: {ids: ids},
                success: function (result) {
                    if (result.error === 0) {
                        if(result.data.body[0]){
                            ...
                            $.post({
                                url: '/businessMgmt/combinationProductConfig/combinationProductHandle/checkTradeStatus.ajax',
                                data: {ids:ids},
                                success: function (result) {
                                    if (result.error === 0) {
        ...
        selectOptionalFundCanSelect: function(item){
            ...
            // if(item.source == 2 && [13, 14, 15, 16, 17].indexOf(Number(fundGroupType)) > -1){ // 外部基金且当前组合类型为13, 14, 15, 16, 17时需要判断同花顺和盈米是否同时可售
                ...
                $.post({
                    url: '/businessMgmt/combinationProductConfig/combinationProductHandle/checkFundCanSelect.ajax',
                    data: data,
                    success: function (result) {
                        if (result.error === 0) {
        ...
        showServiceAllUpdate: function(item){
            ...
            $.post({
                url: '/businessMgmt/combinationProductConfig/combinationProductHandle/getServiceUpdateDialogData.ajax',
                ...
                success: function (result) {
                    if (result.error === 0) {
                        ...
                        this.dataSummary(this.acceptModeList, 'value', 'label', 'acceptModes_serve', this.service.acceptMode);
        ...
        clearServiceAllUpdate: function(){
            ...
            this.service.recommendHoldTime = '01';
            this.service.acceptMode = '0';
            ...
            this.service.riskControl = 'N';
            this.service.riskType = '0';
            this.service.riskTypeList = [];
            this.service.initAmount = '';
            this.service.minRedeemAmount = '';
            this.service.minReserveAmount = '';
            ...
            this.service.riskLevel = '0';
            this.service.riskLevelList = [];
            this.service.commro = '';
            this.service.minChangeAmount = '';
            ...
            // this.dataSummary(this.acceptModeList, 'value', 'label', 'acceptModes_serve', '');
        },
        ...
        showAddNewFundGroupListItem: function(){
            ...
            if(this.service.fundGroupFundListForSelect.length === 0){ // 产品列表为空，需要获取数据
                $.post({
                    url: '/businessMgmt/combinationProductConfig/combinationProductHandle/fundGroupFundListForSelect.ajax',
                    success: function (result) {
                        if (result.error === 0) {
        ...
        serviceAllUpdateSubmit: function(){
            ...
            $.post({
                url: '/businessMgmt/combinationProductConfig/combinationProductHandle/saveServiceAllUpdateDialogData.ajax',
        ...
        checkServiceAllUpdateDialog: function(){
            ...
            if(this.service.isInvestment === 'Y' && this.service.investType === 'G' && !this.service.investCustomers){
                this.showDialog('serviceUpdate', 'info', true, '投顾组合客户不能为空');
                return false;
            }
            if(this.service.isInvestment === 'Y' && this.service.investType === 'G' && !this.service.investPrincipal){
                this.showDialog('serviceUpdate', 'info', true, '名义本金不能为空');
                return false;
            }
            if(this.service.isInvestment === 'Y' && this.service.investType === 'G' && !this.service.investDuration){
                this.showDialog('serviceUpdate', 'info', true, '投资时长不能为空');
                return false;
            }
            if(this.service.isInvestment === 'Y' && !this.service.categoryDescDoc){
                this.showDialog('serviceUpdate', 'info', true, '请上传策略说明书');
                return false;
            }
            if(this.service.isInvestment === 'Y' && !this.service.categoryDescDisplay){
                this.showDialog('serviceUpdate', 'info', true, '请填写策略说明书展示文案');
                return false;
            }
            if(this.service.isInvestment === 'Y' && !this.service.riskDescDoc){
                this.showDialog('serviceUpdate', 'info', true, '请上传风险揭示书');
                return false;
            }
            if(this.service.isInvestment === 'Y' && !this.service.riskDescDisplay){
                this.showDialog('serviceUpdate', 'info', true, '请填写风险揭示书展示文案');
                return false;
            }
            if(this.service.isInvestment === 'Y' && !this.service.investDescDoc){
                this.showDialog('serviceUpdate', 'info', true, '请上传投顾协议');
                return false;
            }
            if(this.service.isInvestment === 'Y' && !this.service.investDescDisplay){
                this.showDialog('serviceUpdate', 'info', true, '请填写投顾协议展示文案');
                return false;
            }
            if(this.service.isInvestment === 'Y' && !this.service.investRiskLevel){
        ...
        showLocalAllUpdate: function(item){
            ...
            this.clearServiceAllUpdate();
            $.post({
                url: '/businessMgmt/combinationProductConfig/combinationProductHandle/getLocalDialogListData.ajax',
                success: function (result) {
                    if (result.error === 0) {
                        for(var prop in result.data){
                            this.service[prop] = result.data[prop];
        ...
        localUpdateSubmit: function(){
            ...
            var params = this.getServiceAllUpdateDialogData();
            $.post({
                url: '/businessMgmt/combinationProductConfig/combinationProductHandle/saveLocalAllUpdateDialogData.ajax',
                data: {
        ...
        showOptionalList: function(dialogName,fundDetailItem){
            ...
            $.post({
                url: '/businessMgmt/combinationProductConfig/combinationProductHandle/checkOptionalFundList.ajax',
                data: params,
                success: function (result) {
                    if(result.error === 0){
        ...
        uploadDoc: function(docType){ // 1-策略说明书,2-风险揭示书,3-投顾协议
            ...
            $.post({
                url: '/businessMgmt/combinationProductConfig/combinationProductHandle/uploadDoc.ajax',
                cache: false,
                data: formdata,
                processData: false,
                contentType: false,
            ...
            $.post({
                ...
                success: function (result) {
                    if (result.error === 0) {
                        ...
                        // 设置渠道赋值
                        this.showDialog('', 'add');
                        this.dataSummary(this.acceptModeList, 'value', 'label', 'acceptMode',this.acceptMode);
                    } else {
                        this.showDialog('', 'info', true, '获取该产品详情信息失败！');
                    }
                }.bind(this)
