...
var vm = new Vue({
    ...
    mounted: function () {
        ...
        // 获取渠道
        $.post({
            url:'/contentMgmt/operationMaterial/addRichSign/getbranchList.ajax',
    ...
    methods: {
        getTagAll:function(){
           ...
           var _this=this;
            $.post({
                url:'/contentMgmt/operationMaterial/addRichSign/getTagAll.ajax',
                data:{
                    parentCategoryId:''
                },
                success: function (result) {
    ...
    // 获取基金列表
        fundList:function(){
            var _this = this;
            $.post({
                url: '/contentMgmt/operationMaterial/addRichSign/fundList.ajax',
                success: function (result) {
                    if (result.error === 0) {
...
// 获取基金经理列表 
        getManagerList: function() {
            var _this = this;
            $.post({
                url: '/contentMgmt/operationMaterial/addRichSign/getManagerList.ajax',
                success: function(result) {
                    if (result.error === 0) {
 ...
 // 主页面数据
        ...
        getTableData: function (currentIndex) {
            ...
            params.categoryId=_this.secondClassifyId;
            ...
            _this.tableData=[];
            $.post({
                url: '/contentMgmt/operationMaterial/addRichSign/getTableData.ajax',
                data: params,
                success: function (result) {
                    $(".cover").hide();
                    if (result.error === 0) {
        ...
        diaInfoCheck: function () {
            ...
            if (!_this.indexDate) {
                this.showDialog('add', 'info', true, '日签日期不能为空');
                return false;
            }
            if (!_this.mediaType) {
     ...
     // 确认删除图片
        ...
        //   }
          ...
          _this.categoryIdList.push(_this.firstClassifyId,_this.secondClassifyId);
        ...
        //   params.branchCodeList =['00'];  //目前写死--渠道码 ,
          ...
          params.indexDate= _this.indexDate?moment(_this.indexDate).format('YYYYMMDD'):'';                   //日签日期
        ...
        //   params.publishTime="20220421"; 
          ...
          let url='';
          if(!_this.updateId){
             url='/contentMgmt/operationMaterial/addRichSign/add.ajax'
          }else{
              params.mediaId=_this.updateId;
             url='/contentMgmt/operationMaterial/addRichSign/update.ajax'
          }
            $.post({
                url:url,
                data:params,
...
// start-现有材料相关操作--弹窗
        ...
        chooseMaterialId:function(materialId,num){
            ...
            console.log(params);
            $.post({
                url: '/contentMgmt/operationMaterial/addRichSign/threeClassifyList.ajax',
                data: params,
                success: function (result) {
                    if (result.error === 0) {
                        console.log(result)
        ...
        searchMaterial:function(currentIndex){
            var _this=this;
                ...
                $.post({
                    url: '/contentMgmt/operationMaterial/addRichSign/searchMaterial.ajax',
                    data: params,
                    success: function (result) {
                        if (result.error === 0) {
                            if(currentIndex){
        ...
        //    _this.categoryIdList.push(_this.firstClassifyId,_this.secondClassifyId);
          ...
          console.log(params);
          $.post({
              url: '/contentMgmt/operationMaterial/addRichSign/getUpdateDetails.ajax',
              data: params,
              success: function (result) {
                  if (result.error === 0) {
        ...
        //    if(_this.mediaType=='file'){
        //     _this.fileUrl=item.mediaDataJson?JSON.parse(item.mediaDataJson).filePath:'';//把数据处理一下
             
        //     }
        //     if(_this.mediaType=='image'){    //file text image  link
        //         _this.imagesUrl.push(item.mediaDataJson?JSON.parse(item.mediaDataJson).imagePath:'');//把数据处理一下
        //     }
        //     if(_this.mediaType=='text'){    //file text image  link
        //         _this.content=item.mediaDataJson?JSON.parse(item.mediaDataJson).content:'';//把数据处理一下
        //         _this.imagesUrl=item.mediaDataJson?JSON.parse(item.mediaDataJson).imagePaths:'';//把数据处理一下
        //     }
        //     if(_this.mediaType=='link'){    //file text image  link
        //     _this.url=item.url;
        //     }
           ...
           _this.indexDate=item.indexDate?moment(item.indexDate).format('YYYY-MM-DD'):''; 
            ...
            $.post({
                ...
                success: function (result) {
                    if (result.error === 0) {
                        ...
                        _this.indexDate=result.data.body[0].indexDate?moment(result.data.body[0].indexDate).format('YYYY-MM-DD'):'';                   //日签日期
...
//修改日签上架状态
        changeStatus:function(item, index){
            var _this = this;
            if (_this.tableData[index].pushStatus == '1') {
                _this.$set(_this.tableData[index], 'pushStatus', '0');
                _this.updatePushStatus(item.mediaId, _this.tableData[index].pushStatus);
            } else {
                _this.$set(_this.tableData[index], 'pushStatus', '1');
                _this.updatePushStatus(item.mediaId, _this.tableData[index].pushStatus);
            }
        },
       updatePushStatus:function(mediaId,pushStatus){
           var _this=this;
           var params={};
               ...
               console.log(params);
            $.post({
                url: '/contentMgmt/operationMaterial/addRichSign/updatePushStatus.ajax',
                data: params,
                success: function (result) {
                    if (result.error === 0) {
                        // setTimeout(function(){
...
//start-删除数据
        ...
        del:function(){
            ...
            console.log(params);
            $.post({
                url: '/contentMgmt/operationMaterial/addRichSign/del.ajax',
                data: params,
                success: function (result) {
                    if (result.error === 0) {
                        setTimeout(function(){
...
//end-删除数据
    ...
    //    一级标签
        getTagList:function(){
            ...
            var params={}
            $.post({
                url:'/contentMgmt/operationMaterial/addRichSign/getTagList.ajax',
                // data:params,
                success: function (result) {
                    if (result.error === 0) {
        ...
        getSecondTagList:function(tagCategoryId,i){
            ...
            console.log(params);
            $.post({
                url:'/contentMgmt/operationMaterial/addRichSign/getSecondTagList.ajax',
                data:params,
                success: function (result) {
                    if (result.error === 0) {
                        console.log(result);               
        ...
        uploadsFile(e) {
            ...
            $(".cover").show();
            $.post({
                url: '/contentMgmt/operationMaterial/addRichSign/uploadFile.ajax',
                type: 'post',
                dataType: 'json',
                data: formdata,
                processData: false,
        ...
        uploadImg:function(e){
            ...
            var reader = new FileReader()
                ...
                reader.onload = (e) => {
                    ...
                    $.post({
                        url: '/contentMgmt/operationMaterial/addRichSign/uploadImg.ajax',
