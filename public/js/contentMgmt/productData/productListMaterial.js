	...
	methods: {
		...
		setData: function (item,flag) {
			...
			if (item) {
				// 要修改的mediaId
				var oldMediaId = '';
				oldMediaId = this.operateData.mediaId;
				this.operateData = {
					...
					modifyBy:item.modifyBy?item.modifyBy:item.createBy,
				...
				if(flag!='2'){//新增的情况我们不需要mediaId，修改非现有材料过来的传自身的mediaId
					item.productMediaRsList&&item.productMediaRsList[0]&&(this.operateData.mediaId =item.productMediaRsList[0].mediaId);
				}else{//修改的情况如果从现有材料点过来的mediaId需要之前记录的oldMediaId，
					oldMediaId&&(this.operateData.mediaId = oldMediaId)
				}
				
				// item.displayImageUrl&&(this.option.img = item.displayImageUrl)
				//类型只要为现有材料就要传
				if(this.operateData.mediaConfigMode=='1'){
					if(flag=='2'){//只要是从现有材料过来的就得传
						this.operateData.boundMediaId = item.mediaId;
					}else{
						console.log('boundMediaId',item.boundMediaId);
						this.operateData.boundMediaId = item.boundMediaId?item.boundMediaId:item.mediaId
					}	
				}
			} 
			$('#dialogBranch').val(this.operateData.branchCodeList).trigger('change');
		...
		add: function (flag) {
			...
			if(data.pushStatus=='0'){//当为非上架状态时传当前默认时间 publishTime 
				...
				data.publishTime = data.publishTime?moment(data.publishTime).format('YYYYMMDDHHmmss'):'';
			}
		...
		checkData: function (item) {
			...
			if (!item.branchCodeList||item.branchCodeList.length===0) {
			...
			// 20220620-对应需求10350:UOP内容管理所有封面图改为非必传
			...
			if(item.pushStatus=='1'&&!item.publishTime){
				this.showDialog('add', 'info', true, '上架状态开启时，上架时间必填')
				return false
			}
			return true
		},
