...
var vm = new Vue({
    ...
    // 获取渠道
        $.post({
            url:'/contentMgmt/productMaterial/investPrefecture/getbranchList.ajax',
    ...
    methods: {
        getTagAll:function(){
           ...
           var _this=this;
            $.post({
                url:'/contentMgmt/productMaterial/investPrefecture/getTagAll.ajax',
                data:{
                    parentCategoryId:''
                },
                success: function (result) {
    ...
    // 获取基金列表
        fundList:function(){
            var _this = this;
            $.post({
                url: '/contentMgmt/productMaterial/investPrefecture/fundList.ajax',
                success: function (result) {
                    if (result.error === 0) {
...
// 获取基金经理列表 
        getManagerList: function() {
            var _this = this;
            $.post({
                url: '/contentMgmt/productMaterial/investPrefecture/getManagerList.ajax',
                success: function(result) {
                    if (result.error === 0) {
 ...
 // 主页面数据
                ...
                success: function (result) {
                    ...
                    if (result.error === 0) {
                        ...
                        if(result.data.body) {
                            ...
                            // _this.modifyBy=result.data.body.createBy; //添加时最新更新人  
    ...
    // 主页面:根据mediaIdList查询parentCategoryId下的素材分类，材料主题
        getThemeList:function(mediaIdList){
            ...
            console.log(params);
            $.post({
                 url: '/contentMgmt/productMaterial/investPrefecture/getThemeList.ajax',
                 data: params,
                 success: function (result) {
                     if (result.error === 0) {
        ...
        showAdd:function(){
            ...
            // if(_this.flag){  //默认取消的再点添加的话不清空
                ...
                _this.timelinessCycleValue='3';            // 素材时效性周期值-默认-3
        ...
        deleteImg:function(){
            var _this=this;
            if(_this.mediaType=='image'){ //图片类型
                _this.imgUrl.splice(_this.indexImg, 1)
            }
            if(_this.mediaType=='text'){ //文字类型
        ...
        //   $('#getTag').val(_this.diaTagManager.split(',')).trigger('change');
        //   let getAllTagList=_this.diaTagManager.split(',').concat(_this.diaTagProduct.split(','));  //关联标签
        ...
        //   params.tagList=_this.diaTagManager.split(',').concat(_this.diaTagProduct.split(','));  //关联标签                                //关联标签

          params.sourceType='00';                   //来源分类：00-uop,01-微信，02-现金宝 
        //   params.sourceTypeChannel='00'   //目前写死             // 来源分类渠道 ,
        ...
        if(_this.mediaType=='text'){    //file text image  link  data.mediaType=='text'&&data.mediaInfo.contentId
            if(_this.contentId){
                params.mediaInfo={   
...
// start-现有材料相关操作--弹窗
        ...
        searchMaterial:function(currentIndex){
            var _this=this;
                ...
                $.post({
                    url: '/contentMgmt/productMaterial/investPrefecture/searchMaterial.ajax',
                    data: params,
                    success: function (result) {
                        if (result.error === 0) {
                            if(currentIndex){
                            ...
                            _this.searchMaterialList = [];
                            _this.currentIndex2 = 0;
                            _this.showDialog('', 'materialShow', false, result.msg);
                        }
                    }
                });      
        },
        ...
        // start-用mediaId调取details数据获取分类id和关联标签的回显
          ...
          console.log(params);
          $.post({
              url: '/contentMgmt/productMaterial/investPrefecture/getUpdateDetails.ajax',
              data: params,
              success: function (result) {
                  if (result.error === 0) {
...
//start-- 修改数据
            ...
            $.post({
                ...
                success: function (result) {
                    if (result.error === 0) {
                        ...
                        _this.firstClassifyId=_this.classifyList[0].categoryId;
                        // _this.threeClassifyId=result.data.body[0].categoryList[2];
                        // console.log(result.data.body[0].categoryList);
                        for(var i=0;i<_this.menuList.length;i++){       //回显获得第三级分类
                            for(var j=0;j<result.data.body[0].categoryList.length;j++){
                                if(_this.menuList[i].categoryId==result.data.body[0].categoryList[j]){
                                    _this.threeClassifyId=result.data.body[0].categoryList[j];
        ...
        // 用mediaId查到当前修改数据的关联标签
        // getUpdateTagList:function(mediaId){
        //     $.post({
        //         url: '/contentMgmt/productMaterial/investPrefecture/getUpdateTagList.ajax',
        //         data: params,
        //         success: function (result) {
        //             if (result.error === 0) {
        //                 if(currentIndex){
...
//start-删除数据
        ...
        del:function(){
            ...
            console.log(params);
            $.post({
                url: '/contentMgmt/productMaterial/investPrefecture/del.ajax',
                data: params,
                success: function (result) {
                    if (result.error === 0) {
                        // _this.getTagAll();
...
//end-删除数据
    ...
    //    一级标签
        getTagList:function(){
            ...
            var params={}
            $.post({
                url:'/contentMgmt/productMaterial/investPrefecture/getTagList.ajax',
                // data:params,
                success: function (result) {
                    if (result.error === 0) {
        ...
        uploadsFile(e) {
            ...
            $(".cover").show();
            $.post({
                url: '/contentMgmt/productMaterial/investPrefecture/uploadFile.ajax',
                type: 'post',
                dataType: 'json',
                data: formdata,
                processData: false,
...
// 上传封面图片
		...
		uploadForServer() {
			...
			var _this = this;
				...
				$.post({
					url: '/contentMgmt/productMaterial/investPrefecture/uploadImgCover.ajax',
					// data: res.compressBase64,
					cache: false,
					data: formdata,
					processData: false,
