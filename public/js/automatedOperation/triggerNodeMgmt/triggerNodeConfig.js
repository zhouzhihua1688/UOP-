new Vue({
    ...
    mounted: function () {
        ...
        //获取节点类型list
        $.post({
            url: '/automatedOperation/triggerNodeMgmt/triggerNodeConfig/queryNodeTypeList.ajax',
    ...
    methods: {
        ...
        getTableData: function (currentIndex) {
            ...
            params.nodeDesc = this.nodeDesc;
            // console.log(params);
            _this.loadingStatus = '数据获取中...';
            $.post({
                url: '/automatedOperation/triggerNodeMgmt/triggerNodeConfig/searchList.ajax',
                data: params,
                success: function (result) {
                    if (result.error === 0) {
        ...
        setAddData: function (obj) {
            ...
            this.dianodeValue = obj.nodeValue ? obj.nodeValue : '';
        ...
        add: function () {
            ...
            if (this.dianodeValue == '') {
                this.showDialog('add', 'info', true, '节点值必须填写！');
                return false;
            }
            var _this = this;
            var params = {};
            ...
            params.nodeValue = this.dianodeValue;
            ...
            var url = '/automatedOperation/triggerNodeMgmt/triggerNodeConfig/';
        ...
        refresh:function () {
            var _this=this;
            $.post({
                url: '/automatedOperation/triggerNodeMgmt/triggerNodeConfig/refresh.ajax',
                success: function (result) {
                    if (result.error === 0) {
                        _this.showDialog('', 'info', false, result.msg);
                    } else {
        ...
        typeTranslate:function (val) {
            if(val){
                this.nodeTypeList.forEach(function (item) {
                    if(item.nodeTypeId==val){
                        val=item.nodeTypeName;
                    }
                })
            }else{
                val='-';
