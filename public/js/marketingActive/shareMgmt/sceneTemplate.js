...
var vm = new Vue({
	...
	methods: {
		...
		operate: function () {
			...
			var list = ['0', '0', '0', '0', '0'];
			this.hasChecked.forEach(function(value){
				list[Number(value)] = '1'
			});
			params.extraSupportType = list.join('');
			console.log(params);
		...
		showUpdate: function (item) {
			...
			this.hasChecked=[];
			var _this = this;
			var extraSupportType = [];
			if(item.extraSupportType){
				extraSupportType = item.extraSupportType.split('');
			}else{
				this.hasChecked=[];
			}
			extraSupportType.forEach(function(item,index){
				if(item==1){
					this.hasChecked.push(String(index));
				}
			},this)
			// var inputBox=$("input[type='checkbox']");
			// for (var i =0;i<inputBox.length;i++) {
			// for (var n = 0; n < extraSupportType.length; n++) {
			//     if (extraSupportType[n] === inputBox[i].value) {
