new Vue({
    ...
    mounted: function () {
        ...
        var _this = this;
        $.post({
            url: '/authorityMgmt/messageCenter/users/getUserInfo.ajax',
    ...
    methods: {
        ...
        showDelete: function () {
            ...
            this.distributedUserViewData.forEach(function (item) {
                ...
                for (var i = 0; i < item.roleList.length; i++) {
                    if (item.roleList[i].name.split('|')[0] == '消息中心') {
        ...
        deleteRole: function () {
            ...
            $.post({
                url: '/authorityMgmt/messageCenter/users/deleteUserRole.ajax',
                data: params,
                success: function (result) {
                    if (result.error == 0) {
                        _this.deleteId = '';
        ...
        checkTree: function (item) {
            ...
            $.post({
                url: '/authorityMgmt/messageCenter/users/checkTree.ajax',
        ...
        updateRole: function (item) {
            ...
            if (this.AllRoleList.length === 0) {
                $.post({
                    url: '/authorityMgmt/messageCenter/users/getRoleInfo.ajax',
                    async: false,
                    success: function (result) {
        ...
        distributeRole: function () {
            ...
            var _this = this;
            $.post({
                url: '/authorityMgmt/messageCenter/users/distributeRole.ajax',
                data: params,
                success: function (result) {
                    if (result.error === 0) {
                        _this.getUserInfo();
