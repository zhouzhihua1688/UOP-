...
new Vue({
    ...
    watch: {
		...
		'opreateData.partiEndDate':function(newval,oldval){
			if(this.modifyStatus=='0'&&newval){//只有新增的时候自动填入
				// 获取当前选择的发车类型typeDesc是否能转化为数字类型；
				var formatNewval = newval.replace(/-/g,'/');
				var obj;
				// 自动填入,运作起始日day1,观察起始日day2,运作到期日day3
				var day1 = new Date(formatNewval).getTime()+1*1000*60*60*24;  //运作起始日=参与结束日+1
				var day2 = new Date(formatNewval).getTime()+90*1000*60*60*24; //观察起始日=参与结束日+90
				var day3 = new Date(formatNewval).getTime()+182*1000*60*60*24;  //运作到期日=参与结束日+182(182天为默认,如果下方obj获取到的typeDesc字段可转化为数字，则day3=Number(typeDesc))
				...
				if(obj&&!isNaN(obj.typeDesc)){//能转化为数值时
					...
					day3 = new Date(formatNewval).getTime()+Number(obj.typeDesc)*1000*60*60*24;
				}
    ...
    methods: {
		...
		initData:function(item){
			if(item){
				...
				this.opreateData.endDefaultOperation='01';
				this.opreateData.endOptionalOperationList=[];
				this.opreateData.targetReturnPercent = 5;
		...
		checkData:function(item){
			...
			if(item.targetReturnPercent===''){
				this.showDialog('add','info',true,'请选择填写目标年化收益率')
				return false
			}
			if(item.targetReturnPercent<5){
				this.showDialog('add','info',true,'目标年化收益率不能小于5')
				return false
			}
			...
			if(item.isEndWin=='1'&&!item.endWinDate){
				this.showDialog('add','info',true,'请填写止盈日期')
				return false
			}
			return true
		},
