new Vue({
    ...
    data: {
        ...
        diaimageCacheFlag: "",
        diaremark: "",
        diabtnBackgroundColor: "",
        diabtnSubName: "",
        diarecommendTp: "",
        diarecommendMode: "1",
        diamarketDisplayMode: "",
        diamarketContent: "",
        diaredDotDisplayMode: "",
        diaredDotApiPath: "",
        diasupportos: "",
        diarecommendContent: "",
        ...
        elementShow:{
            ...
            diaremark:true,
            diabtnBackgroundColor:true,
            diabtnSubName:true,
            diarecommendTp:true,
            diarecommendMode: true,
            diamarketDisplayMode:true,
            diamarketContent:true,
            diaredDotDisplayMode:true,
            diaredDotApiPath:true,
            diasupportos:true,
            diarecommendContent:true,
            diaimageUrl:true
        },
        searchChannelId:'',
        //主表格分页数据
    ...
    mounted: function () {
        ...
        //查询推荐类型接口
        $.post({
            url: '/recommendSystem/recommendSystemTemplateConfig/AppBaseConfig/queryRecommendTp.ajax',
            success: function (result) {
                if (result.error === 0) {
                    if (result.data && result.data.length > 0) {
                        _this.recommendTpList = result.data;
                    } else {
                        _this.showDialog('', 'info', false, '推荐类型无数据');
                    }
                } else {
    ...
    methods: {
        ...
        getOneDetail: function (objconfigId) {
            ...
            params.btnId = objconfigId;
            // params.channelId=this.searchChannelId;
            // console.log(params);
            _this.loadingStatus = '数据获取中...';
            $.post({
                url: '/recommendSystem/recommendSystemTemplateConfig/AppBaseConfig/queryInfo.ajax',
                data: params,
                success: function (result) {
                    if (result.error === 0) {
        ...
        getViewData: function (val,once) {
            ...
            params.contentTp = 'custombtn';
            val && (params.temId = val);
            $.post({
                url: '/recommendSystem/recommendSystemTemplateConfig/AppBaseConfig/viewList.ajax',
                data: params,
                success: function (result) {
                    if (result.error === 0) {
                        if (result.data.rows.length > 0) {
        ...
        getTableData: function (currentIndex) {
            ...
            params.pageSize = this.pageMaxNum;
            params.btnName = this.btnName;
            params.btnKey = this.btnKey;
            params.btnSubName = this.btnSubName;
            params.channelId=this.searchChannelId;
            // console.log(params);
            _this.loadingStatus = '数据获取中...';
            $.post({
        ...
        setAddData: function (obj) {
            this.diabtnKey = obj.btnKey ? obj.btnKey : '';
            ...
            this.diabtnBackgroundColor = obj.btnBackgroundColor ? obj.btnBackgroundColor : '';
            this.diabtnSubName = obj.btnSubName ? obj.btnSubName : '';
            this.diarecommendTp = obj.recommendTp ? obj.recommendTp : '';
            this.diarecommendMode = obj.recommendMode ? obj.recommendMode : '';
            this.diamarketDisplayMode = obj.marketDisplayMode ? obj.marketDisplayMode : '';
            this.diamarketContent = obj.marketContent ? obj.marketContent : '';
            this.diaredDotDisplayMode = obj.redDotDisplayMode ? obj.redDotDisplayMode : '';
            this.diaredDotApiPath = obj.redDotApiPath ? obj.redDotApiPath : '';
            this.diasupportos = obj.supportos ? obj.supportos : '';
            this.diarecommendContent = obj.recommendContent ? obj.recommendContent : '';
        ...
        showAdd: function () {
            ...
            this.setAddData({recommendMode: '1'});
            this.showDialog('', 'add');
            $("#previewImg").show();
        },
        showUpdate: function (item) {
            if(item.previewTemid){
                this.getViewData(item.previewTemid,1);
            }else{
                for(var ele in this.elementShow ){
                    this.elementShow[ele]=true;
                }
        ...
        add: function () {
            ...
            params.btnBackgroundColor = this.diabtnBackgroundColor;
            params.btnSubName = this.diabtnSubName;
            params.recommendTp = this.diarecommendTp;
            params.recommendMode = this.diarecommendMode;
            params.marketDisplayMode = this.diamarketDisplayMode;
            params.marketContent = this.diamarketContent;
            params.redDotDisplayMode = this.diaredDotDisplayMode;
            params.redDotApiPath = this.diaredDotApiPath;
            params.supportos = this.diasupportos;
            params.channelId = this.channelId;
            params.previewTemid = this.elementCollection;
            params.imageUrl = $('#uploadInput1').val();
            (this.diarecommendTp == '2') && (params.recommendContent = this.diarecommendContent);
            this.isUpdate && (params.btnId = this.updateId);
            !this.isUpdate && (params.createBy = this.userId);
            this.isUpdate && (params.modifyBy = this.userId);
            var url = '/recommendSystem/recommendSystemTemplateConfig/AppBaseConfig/';
        ...
        uploadPic1: function () {
            ...
            var _this = this;
            $.ajaxFileUpload({
                url: '/recommendSystem/recommendSystemTemplateConfig/AppBaseConfig/upload1.ajax',
                type: 'POST',
                dataType: 'json',
        ...
        enableOrDisable: function (item) {
            var _this = this;
            $.post({
                url: '/recommendSystem/recommendSystemTemplateConfig/AppBaseConfig/enable.ajax',
        ...
        deleteParams: function (item) {
            this.deleteId = item.btnId;
            this.showDialog('', 'delete1', false);
        },
        deleteConfirm: function () {
            var _this = this;
            $.post({
                url: '/recommendSystem/recommendSystemTemplateConfig/AppBaseConfig/delete.ajax',
                data: {btnId: _this.deleteId},
                success: function (result) {
                    if (result.error === 0) {
                        _this.showDialog('', 'info', false, '操作成功');
                        _this.getTableData(_this.currentIndex);
        ...
        fresh: function () {
            var _this = this;
            $.post({
                url: '/recommendSystem/recommendSystemTemplateConfig/AppBaseConfig/fresh.ajax',
                success: function (result) {
                    if (result.error === 0) {
                        _this.showDialog('', 'info', false, '操作成功');
                    } else {
        ...
        configMgmt:function(){
            ...
            for (var j = 0; j < this.tableData.length; j++) {
                if (this.tableData[j].check) {
                    this.tableData[j].objconfigId=this.tableData[j].btnId;
                    this.tableData[j].startTime='';
                    this.tableData[j].endTime='';
                    this.tableData[j].modifyTime='';
                    this.tableData[j].typeKey='custombtn';
            ...
            if(!window.localStorage){
                ...
                var url="/recommendSystem/recommendSystemConfigMgmt/positionTheThemeMgmt.html?pageType=Modify&contentTp=custombtn&channelId="+channelIds[0]
                storage.setItem("custombtn",JSON.stringify(ids));
                window.open(url);
            }
        ...
        configThis:function(item){
            ...
            if(!window.localStorage){
                ...
                var storage=window.localStorage;
                var url="/recommendSystem/recommendSystemConfigMgmt/positionTheThemeMgmt.html?pageType=Modify&contentTp=custombtn&channelId="+channelIds+'&objconfigId='+ids[0].objconfigId;
                storage.setItem("custombtn",JSON.stringify(ids));
                window.open(url);
            }
        },
