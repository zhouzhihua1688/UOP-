new Vue({
    ...
    data: {
        ...
        elementShow:{
            ...
            shareImgUrl: true,
            diaremark:true,
            diadisplayStartDtin:true,
            diadisplayEndDtin:true,
            diaimageUrl:true
        },
        searchChannelId:'',
        //主表格分页数据
    ...
    methods: {
        ...
        getOneDetail: function (objconfigId) {
            ...
            params.loadPageConfigId = objconfigId;
            // params.channelId=this.searchChannelId;
            // console.log(params);
            _this.loadingStatus = '数据获取中...';
            $.post({
                url: '/recommendSystem/recommendSystemTemplateConfig/loadPageConfigMgmt/queryInfo.ajax',
                data: params,
        ...
        getViewData: function (val,once) {
            ...
            params.contentTp = 'loadpage';
            val && (params.temId = val);
            $.post({
                url: '/recommendSystem/recommendSystemTemplateConfig/loadPageConfigMgmt/viewList.ajax',
                data: params,
                success: function (result) {
                    if (result.error === 0) {
                        if (result.data.rows.length > 0) {
        ...
        setAddData: function (obj) {
            if(obj.displayStartDt){
                obj.displayStartDt=this.formatTime(obj.displayStartDt);
            }
            if(obj.displayEndDt){
                obj.displayEndDt=this.formatTime(obj.displayEndDt);
            }
            this.diaurl = obj.url ? obj.url : '';
			this.diaimageCacheFlag = obj.imageCacheFlag ? obj.imageCacheFlag : '0';
            this.diaholdingTime = obj.holdingTime ? obj.holdingTime : '';
            ...
            $('#displayStartDtin').val(obj.displayStartDt ? obj.displayStartDt : '');
            $('#displayEndDtin').val(obj.displayEndDt ? obj.displayEndDt : '');
            $('#uploadInput1').val(obj.imageUrl ? obj.imageUrl : '');
            $('#uploadInput2').val(obj.shareImgUrl ? obj.shareImgUrl : '');
        },
        showAdd: function () {
        ...
        showUpdate: function (item) {
            ...
            this.pagePreviewImgurl = '';
            this.isUpdate = true;
            this.updateId = item.loadPageConfigId;
        ...
        add: function () {
            ...
            if (this.diaInfoCheck()) {
                var params = {};
                    ...
                    params.holdingTime= this.diaholdingTime;
                    params.remark= this.diaremark;
                    params.displayStartDtin= $('#displayStartDtin').val();
                    params.displayEndDtin= $('#displayEndDtin').val();
                    params.imageUrl= $('#uploadInput1').val();
                    params.shareImgUrl= $('#uploadInput2').val();
                    params.channelId = this.channelId;
                    params.previewTemid = this.elementCollection;

                this.isUpdate && (params.loadPageConfigId = this.updateId);
                this.isUpdate && (params.modifyBy = this.userId);
                var url = '/recommendSystem/recommendSystemTemplateConfig/loadPageConfigMgmt/';
        ...
        uploadPic1: function () {
            ...
            var _this = this;
            $.ajaxFileUpload({
                url: '/recommendSystem/recommendSystemTemplateConfig/loadPageConfigMgmt/upload1.ajax',
                type: 'POST',
                dataType: 'json',
        ...
        diaInfoCheck: function () {
            if(!/^\d+$/.test(this.diaholdingTime)&&this.diaholdingTime){
                this.showDialog('add', 'info', true, '停留时间必须为整数');
                return false;
            }
        ...
        enableOrDisable: function (item) {
            var _this = this;
            $.post({
                url: '/recommendSystem/recommendSystemTemplateConfig/loadPageConfigMgmt/enable.ajax',
                data: {loadPageConfigId: item.loadPageConfigId},
                success: function (result) {
                    if (result.error === 0) {
        ...
        deleteParams: function (item) {
            this.deleteId= item.loadPageConfigId;
            this.showDialog('', 'delete1', false);
        },
        deleteConfirm: function () {
            var _this = this;
            $.post({
                url: '/recommendSystem/recommendSystemTemplateConfig/loadPageConfigMgmt/delete.ajax',
                data: {LoadpageId: _this.deleteId},
                success: function (result) {
                    if (result.error === 0) {
                        _this.showDialog('', 'info', false, '操作成功');
                        _this.getTableData(_this.currentIndex);
        ...
        fresh:function () {
            var _this=this;
            $.post({
                url: '/recommendSystem/recommendSystemTemplateConfig/loadPageConfigMgmt/fresh.ajax',
                success: function (result) {
                    if (result.error === 0) {
                        _this.showDialog('', 'info', false, '操作成功');
                    } else {
        ...
        configMgmt:function(){
            ...
            for (var j = 0; j < this.tableData.length; j++) {
                if (this.tableData[j].check) {
                    this.tableData[j].objconfigId=this.tableData[j].loadPageConfigId;
                    this.tableData[j].startTime='';
                    this.tableData[j].endTime='';
                    this.tableData[j].modifyTime='';
                    this.tableData[j].typeKey='loadpage';
            ...
            if(!window.localStorage){
                ...
                var url="/recommendSystem/recommendSystemConfigMgmt/positionTheThemeMgmt.html?pageType=Modify&contentTp=loadpage&channelId="+channelIds[0]
                storage.setItem("loadpage",JSON.stringify(ids));
                window.open(url);
            }
        ...
        configThis:function(item){
            ...
            var obj={
                objconfigId:item.loadPageConfigId,
                startTime:'',
                endTime:'',
                modifyTime:'',
                typeKey:'loadpage',
            ...
            if(!window.localStorage){
                ...
                var storage=window.localStorage;
                var url="/recommendSystem/recommendSystemConfigMgmt/positionTheThemeMgmt.html?pageType=Modify&contentTp=loadpage&channelId="+channelIds+'&objconfigId='+ids[0].objconfigId;
                storage.setItem("loadpage",JSON.stringify(ids));
                window.open(url);
            }
        },
