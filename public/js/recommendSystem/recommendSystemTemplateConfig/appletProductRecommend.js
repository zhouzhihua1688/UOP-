new Vue({
    ...
    methods: {
        ...
        getOneDetail: function (objconfigId) {
            ...
            params.weProdId = objconfigId;
            // params.channelId=this.searchChannelId;
            // console.log(params);
            _this.loadingStatus = '数据获取中...';
            $.post({
                url: '/recommendSystem/recommendSystemTemplateConfig/appletProductRecommend/queryInfo.ajax',
                data: params,
        ...
        getViewData: function (val,once) {
            ...
            params.contentTp = 'weprod';
            val && (params.temId = val);
            $.post({
                url: '/recommendSystem/recommendSystemTemplateConfig/appletProductRecommend/viewList.ajax',
                data: params,
                success: function (result) {
                    if (result.error === 0) {
                        if (result.data.rows.length > 0) {
        ...
        getTableData: function (currentIndex) {
            ...
            params.pageSize = this.pageMaxNum;
            params.sceneFundId = this.sceneFundId;
        ...
        setAddData: function (obj) {
            this.diasceneFundId = obj.sceneFundId ? obj.sceneFundId : '';
            ...
            $('#uploadInput1').val(obj.sceneIconUrl ? obj.sceneIconUrl : '');
        },
        showAdd: function () {
        ...
        add: function () {
            ...
            var _this = this;
            var params = {};
            params.sceneFundId = this.diasceneFundId;
            ...
            params.sceneIconUrl = $('#uploadInput1').val();
            this.isUpdate && (params.weProdId = this.updateId);
            !this.isUpdate && (params.createBy = this.userId);
            this.isUpdate && (params.modifyBy = this.userId);
            var url = '/recommendSystem/recommendSystemTemplateConfig/appletProductRecommend/';
        ...
        uploadPic1: function () {
            ...
            var _this = this;
            $.ajaxFileUpload({
                url: '/recommendSystem/recommendSystemTemplateConfig/appletProductRecommend/upload1.ajax',
                type: 'POST',
                dataType: 'json',
        ...
        deleteParams: function (item) {
            this.deleteId = item.weProdId;
            this.showDialog('', 'delete1', false);
        },
        deleteConfirm: function () {
            var _this = this;
            $.post({
                url: '/recommendSystem/recommendSystemTemplateConfig/appletProductRecommend/delete.ajax',
                data: {weProdId: this.deleteId},
                success: function (result) {
                    if (result.error === 0) {
        ...
        fresh: function () {
            var _this = this;
            $.post({
                url: '/recommendSystem/recommendSystemTemplateConfig/appletProductRecommend/fresh.ajax',
                success: function (result) {
                    if (result.error === 0) {
                        _this.showDialog('', 'info', false, '操作成功');
                    } else {
        ...
        configMgmt:function(){
            ...
            for (var j = 0; j < this.tableData.length; j++) {
                if (this.tableData[j].check) {
                    this.tableData[j].objconfigId=this.tableData[j].weProdId;
                    this.tableData[j].startTime='';
                    this.tableData[j].endTime='';
                    this.tableData[j].modifyTime='';
                    this.tableData[j].typeKey='weprod';
            ...
            if(!window.localStorage){
                ...
                var url="/recommendSystem/recommendSystemConfigMgmt/positionTheThemeMgmt.html?pageType=Modify&contentTp=weprod&channelId="+channelIds[0]
                storage.setItem("weprod",JSON.stringify(ids));
                window.open(url);
            }
        ...
        configThis:function(item){
            ...
            if(!window.localStorage){
                ...
                var storage=window.localStorage;
                var url="/recommendSystem/recommendSystemConfigMgmt/positionTheThemeMgmt.html?pageType=Modify&contentTp=weprod&channelId="+channelIds+'&objconfigId='+ids[0].objconfigId;
                storage.setItem("weprod",JSON.stringify(ids));
                window.open(url);
            }
        },
