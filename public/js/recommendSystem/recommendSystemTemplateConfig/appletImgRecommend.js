new Vue({
    ...
    methods: {
        ...
        getOneDetail: function (objconfigId) {
            ...
            params.weBannerId = objconfigId;
            // params.channelId=this.searchChannelId;
            // console.log(params);
            _this.loadingStatus = '数据获取中...';
            $.post({
                url: '/recommendSystem/recommendSystemTemplateConfig/appletImgRecommend/queryInfo.ajax',
                data: params,
        ...
        getViewData: function (val,once) {
            ...
            params.contentTp = 'webanner';
            val && (params.temId = val);
            $.post({
                url: '/recommendSystem/recommendSystemTemplateConfig/appletImgRecommend/viewList.ajax',
                data: params,
                success: function (result) {
                    if (result.error === 0) {
                        if (result.data.rows.length > 0) {
        ...
        getTableData: function (currentIndex) {
            ...
            params.pageSize = this.pageMaxNum;
            params.bannerTitle = this.bannerTitle;
        ...
        setAddData: function (obj) {
            ...
            this.diabannerTitle = obj.bannerTitle ? obj.bannerTitle : '';
            this.channelId = obj.channelId ? obj.channelId : '';
            this.elementCollection = obj.previewTemid ? obj.previewTemid : '';
            $('#uploadInput1').val(obj.bannerUrl ? obj.bannerUrl : '');
            $('#uploadInput2').val(obj.detailUrl ? obj.detailUrl : '');
        },
        showAdd: function () {
        ...
        showUpdate: function (item) {
            if(item.previewTemid){
                this.getViewData(item.previewTemid);
            }else{
                for(var ele in this.elementShow ){
                    this.elementShow[ele]=true;
                }
        ...
        add: function () {
            ...
            params.bannerTitle = this.diabannerTitle;
            params.bannerUrl = $('#uploadInput1').val();
            params.detailUrl = $('#uploadInput2').val();
            params.channelId = this.channelId;
            params.previewTemid = this.elementCollection;
            this.isUpdate && (params.weBannerId = this.updateId);
            !this.isUpdate && (params.createBy = this.userId);
            var url = '/recommendSystem/recommendSystemTemplateConfig/appletImgRecommend/';
        ...
        uploadPic1: function () {
            ...
            var _this = this;
            $.ajaxFileUpload({
                url: '/recommendSystem/recommendSystemTemplateConfig/appletImgRecommend/upload1.ajax',
                type: 'POST',
                dataType: 'json',
        ...
        uploadPic2: function () {
            ...
            var _this = this;
            $.ajaxFileUpload({
                url: '/recommendSystem/recommendSystemTemplateConfig/appletImgRecommend/upload2.ajax',
                type: 'POST',
                dataType: 'json',
        ...
        deleteParams: function (item) {
            this.deleteId = item.weBannerId;
            this.showDialog('', 'delete1', false);
        },
        deleteConfirm: function () {
            var _this = this;
            $.post({
                url: '/recommendSystem/recommendSystemTemplateConfig/appletImgRecommend/delete.ajax',
                data: {weBannerId: this.deleteId},
                success: function (result) {
                    if (result.error === 0) {
        ...
        fresh: function () {
            var _this = this;
            $.post({
                url: '/recommendSystem/recommendSystemTemplateConfig/appletImgRecommend/fresh.ajax',
                success: function (result) {
                    if (result.error === 0) {
                        _this.showDialog('', 'info', false, '操作成功');
                    } else {
        ...
        configMgmt:function(){
            ...
            for (var j = 0; j < this.tableData.length; j++) {
                if (this.tableData[j].check) {
                    this.tableData[j].objconfigId=this.tableData[j].weBannerId;
                    this.tableData[j].startTime='';
                    this.tableData[j].endTime='';
                    this.tableData[j].modifyTime='';
                    this.tableData[j].typeKey='webanner';
            ...
            if(!window.localStorage){
                ...
                var url="/recommendSystem/recommendSystemConfigMgmt/positionTheThemeMgmt.html?pageType=Modify&contentTp=webanner&channelId="+channelIds[0]
                storage.setItem("webanner",JSON.stringify(ids));
                window.open(url);
            }
        ...
        configThis:function(item){
            ...
            if(!window.localStorage){
                ...
                var storage=window.localStorage;
                var url="/recommendSystem/recommendSystemConfigMgmt/positionTheThemeMgmt.html?pageType=Modify&contentTp=webanner&channelId="+channelIds+'&objconfigId='+ids[0].objconfigId;
                storage.setItem("webanner",JSON.stringify(ids));
                window.open(url);
            }
        },
