new Vue({
    ...
    methods: {
        ...
        getOneDetail: function (objconfigId) {
            ...
            $.post({
                url: '/recommendSystem/recommendSystemTemplateConfig/wxImgAdConfig/queryInfo.ajax',
                data: params,
        ...
        getViewData: function (val,once) {
            ...
            params.contentTp = 'wx_adImg';
            val && (params.temId = val);
            $.post({
                url: '/recommendSystem/recommendSystemTemplateConfig/wxImgAdConfig/viewList.ajax',
                data: params,
                success: function (result) {
                    if (result.error === 0) {
                        if (result.data.rows.length > 0) {
        ...
        add: function () {
            ...
            var url = '/recommendSystem/recommendSystemTemplateConfig/wxImgAdConfig/';
        ...
        uploadPic1: function () {
            ...
            var _this = this;
            $.ajaxFileUpload({
                url: '/recommendSystem/recommendSystemTemplateConfig/wxImgAdConfig/upload1.ajax',
                type: 'POST',
                dataType: 'json',
        ...
        deleteConfirm: function () {
            var _this = this;
            $.post({
                url: '/recommendSystem/recommendSystemTemplateConfig/wxImgAdConfig/delete.ajax',
        ...
        fresh: function () {
            var _this = this;
            $.post({
                url: '/recommendSystem/recommendSystemTemplateConfig/wxImgAdConfig/fresh.ajax',
                success: function (result) {
                    if (result.error === 0) {
                        _this.showDialog('', 'info', false, '操作成功');
                    } else {
        ...
        configMgmt:function(){
            ...
            for (var j = 0; j < this.tableData.length; j++) {
                if (this.tableData[j].check) {
                    ...
                    this.tableData[j].typeKey='wx_adImg';
            ...
            if(!window.localStorage){
                ...
                var url="/recommendSystem/recommendSystemConfigMgmt/positionTheThemeMgmt.html?pageType=Modify&contentTp=wx_adImg&channelId="+channelIds[0]
                storage.setItem("wx_adImg",JSON.stringify(ids));
                window.open(url);
            }
        ...
        configThis:function(item){
            ...
            if(!window.localStorage){
                ...
                var storage=window.localStorage;
                var url="/recommendSystem/recommendSystemConfigMgmt/positionTheThemeMgmt.html?pageType=Modify&contentTp=wx_adImg&channelId="+channelIds+'&objconfigId='+ids[0].objconfigId;
                storage.setItem("wx_adImg",JSON.stringify(ids));
                window.open(url);
            }
        },
