new Vue({
    ...
    methods: {
        ...
        getOneDetail: function (objconfigId) {
            ...
            $.post({
                url: '/recommendSystem/recommendSystemTemplateConfig/wxProductTemplateConfig/queryInfo.ajax',
                data: params,
        ...
        getViewData: function (val,once) {
            ...
            params.contentTp = 'wx_product';
            val && (params.temId = val);
            $.post({
                url: '/recommendSystem/recommendSystemTemplateConfig/wxProductTemplateConfig/viewList.ajax',
                data: params,
                success: function (result) {
                    if (result.error === 0) {
                        if (result.data.rows.length > 0) {
        ...
        setAddData: function (obj) {
            ...
            $('#prdStartDate').val(obj.prdStartDate ? obj.prdStartDate : '');
            $('#prdEndDate').val(obj.prdEndDate ? obj.prdEndDate : '');
        },
        showAdd: function () {
        ...
        add: function () {
            ...
            var url = '/recommendSystem/recommendSystemTemplateConfig/wxProductTemplateConfig/';
        ...
        deleteConfirm: function (item) {
            var _this = this;
            $.post({
                url: '/recommendSystem/recommendSystemTemplateConfig/wxProductTemplateConfig/delete.ajax',
        ...
        fresh:function () {
            var _this=this;
            $.post({
                url: '/recommendSystem/recommendSystemTemplateConfig/wxProductTemplateConfig/fresh.ajax',
                success: function (result) {
                    if (result.error === 0) {
                        _this.showDialog('', 'info', false, '操作成功');
                    } else {
        ...
        configMgmt:function(){
            ...
            for (var j = 0; j < this.tableData.length; j++) {
                if (this.tableData[j].check) {
                    ...
                    this.tableData[j].typeKey='wx_product';
            ...
            if(!window.localStorage){
                ...
                var url="/recommendSystem/recommendSystemConfigMgmt/positionTheThemeMgmt.html?pageType=Modify&contentTp=wx_product&channelId="+channelIds[0]
                storage.setItem("wx_product",JSON.stringify(ids));
                window.open(url);
            }
        ...
        configThis:function(item){
            ...
            if(!window.localStorage){
                ...
                var storage=window.localStorage;
                var url="/recommendSystem/recommendSystemConfigMgmt/positionTheThemeMgmt.html?pageType=Modify&contentTp=wx_product&channelId="+channelIds+'&objconfigId='+ids[0].objconfigId;
                storage.setItem("wx_product",JSON.stringify(ids));
                window.open(url);
            }
        },
