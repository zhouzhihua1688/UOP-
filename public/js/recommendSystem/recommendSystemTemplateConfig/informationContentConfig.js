new Vue({
    ...
    data: {
        ...
        elementShow:{
            ...
            diaadviceCategory: true,
            diaadviceRelateId: true
        },
        searchChannelId:'',
        //主表格分页数据
        currentIndex: 0,
    ...
    mounted: function () {
        ...
        //查询展示样式接口
        $.post({
            url: '/recommendSystem/recommendSystemTemplateConfig/informationContentConfig/adviceDisplayStyle.ajax',
        ...
        //分页查询资讯类型接口
        $.post({
            url: '/recommendSystem/recommendSystemTemplateConfig/informationContentConfig/adviceTpListQuery.ajax',
            success: function (result) {
                if (result.error === 0) {
    ...
    methods: {
        ...
        getOneDetail: function (objconfigId) {
            ...
            params.advconfigId = objconfigId;
            // params.channelId=this.searchChannelId;
            // console.log(params);
            _this.loadingStatus = '数据获取中...';
            $.post({
                url: '/recommendSystem/recommendSystemTemplateConfig/informationContentConfig/queryInfo.ajax',
                data: params,
                success: function (result) {
                    if (result.error === 0) {
        ...
        getViewData: function (val,once) {
            ...
            params.contentTp = 'advice';
            val && (params.temId = val);
            $.post({
                url: '/recommendSystem/recommendSystemTemplateConfig/informationContentConfig/viewList.ajax',
                data: params,
                success: function (result) {
                    if (result.error === 0) {
                        if (result.data.rows.length > 0) {
        ...
        setAddData: function (obj) {
            if (obj.publicTime) {
                obj.publicTime = this.formatTime(obj.publicTime)
            }
            ...
            this.diaadviceCategory = obj.adviceCategory ? obj.adviceCategory : '';
            this.diaadviceRelateId = obj.adviceRelateId ? obj.adviceRelateId : '';
            $('#publicTimein').val(obj.publicTime ? obj.publicTime : '');
        ...
        add: function () {
            ...
            params.adviceCategory = this.diaadviceCategory;
            params.adviceRelateId = this.diaadviceRelateId;
            ...
            var url = '/recommendSystem/recommendSystemTemplateConfig/informationContentConfig/';
        ...
        uploadPic1: function () {
            ...
            var _this = this;
            $.ajaxFileUpload({
                url: '/recommendSystem/recommendSystemTemplateConfig/informationContentConfig/upload1.ajax',
                type: 'POST',
                dataType: 'json',
        ...
        uploadPic2: function () {
            ...
            var _this = this;
            $.ajaxFileUpload({
                url: '/recommendSystem/recommendSystemTemplateConfig/informationContentConfig/upload2.ajax',
                type: 'POST',
                dataType: 'json',
        ...
        deleteConfirm: function () {
            var _this = this;
            $.post({
                url: '/recommendSystem/recommendSystemTemplateConfig/informationContentConfig/delete.ajax',
                data: {advconfigId: _this.deleteId},
                success: function (result) {
                    if (result.error === 0) {
        ...
        fresh: function () {
            var _this = this;
            $.post({
                url: '/recommendSystem/recommendSystemTemplateConfig/informationContentConfig/fresh.ajax',
                success: function (result) {
                    if (result.error === 0) {
                        _this.showDialog('', 'info', false, '操作成功');
                    } else {
        ...
        configMgmt:function(){
            ...
            for (var j = 0; j < this.tableData.length; j++) {
                if (this.tableData[j].check) {
                    this.tableData[j].objconfigId=this.tableData[j].advconfigId;
                    this.tableData[j].startTime='';
                    this.tableData[j].endTime='';
                    this.tableData[j].modifyTime='';
                    this.tableData[j].typeKey='advice';
            ...
            if(!window.localStorage){
                ...
                var url="/recommendSystem/recommendSystemConfigMgmt/positionTheThemeMgmt.html?pageType=Modify&contentTp=advice&channelId="+channelIds[0]
                storage.setItem("advice",JSON.stringify(ids));
                window.open(url);
            }
        ...
        configThis:function(item){
            ...
            if(!window.localStorage){
                ...
                var storage=window.localStorage;
                var url="/recommendSystem/recommendSystemConfigMgmt/positionTheThemeMgmt.html?pageType=Modify&contentTp=advice&channelId="+channelIds+'&objconfigId='+ids[0].objconfigId;
                storage.setItem("advice",JSON.stringify(ids));
                window.open(url);
            }
        },
