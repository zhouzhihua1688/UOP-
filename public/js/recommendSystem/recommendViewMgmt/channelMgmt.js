new Vue({
    ...
    mounted: function () {
        ...
        dialogs.forEach(function (id) {
         ...
         //获取渠道列表
        $.post({
            url: '/recommendSystem/recommendViewMgmt/channelMgmt/channelMenu.ajax',
    ...
    methods: {
        ...
        getTableData: function (currentIndex) {
            ...
            params.channelRemark = this.channelRemark;
            params.channelId=this.searchChannelId;
            _this.loadingStatus = '数据获取中...';
            $.post({
                url: '/recommendSystem/recommendViewMgmt/channelMgmt/queryInfo.ajax',
                data: params,
                success: function (result) {
                    if (result.error === 0) {
                        console.log(result);
        ...
        setAddData: function (obj) {
            this.diachannelName = obj.channelName ? obj.channelName : '';
            this.diachannelRemark = obj.channelRemark ? obj.channelRemark : '';
        },
        showAdd: function () {
            this.isUpdate = false;
            this.setAddData({});
        ...
        add: function () {
            var _this = this;
            var params = {};
            if(this.diachannelName==''){
                _this.showDialog('add', 'info', true, '名称必须填写！');
                return;
            }
            params.channelName = _this.diachannelName;
            params.channelRemark = _this.diachannelRemark;
            params.createBy = _this.userId;
            _this.isUpdate&&(params.channelId=_this.updateId);
            var url = '/recommendSystem/recommendViewMgmt/channelMgmt/';
        ...
        deleteParams: function (item) {
            this.deleteId1=item.channelId;
            this.showDialog('', 'delete1', false);
        },
        deleteConfirm:function () {
            var _this = this;
            $.post({
                url: '/recommendSystem/recommendViewMgmt/channelMgmt/delete.ajax',
                data: {channelId: _this.deleteId1},
                success: function (result) {
                    if (result.error === 0) {
        ...
        var _this=this;
            var str='';
            if(val){
                _this.channelMenu.forEach(function (item) {
