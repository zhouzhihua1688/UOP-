...
module.exports = function (app) {
    ...
    app.post('/authorityMgmt/allChannels/roles/roleData.ajax', (req, res, next) => {
        let option = {
            pageUrl: '/authorityMgmt/allChannels/roles/roleData.ajax',
            req: req,
    ...
    app.post('/authorityMgmt/allChannels/roles/checkTree.ajax', (req, res, next) => {
        let option = {
            pageUrl: '/authorityMgmt/allChannels/roles/checkTree.ajax',
            req: req,
    ...
    app.post('/authorityMgmt/allChannels/roles/deleteRole.ajax', (req, res, next) => {
        let option = {
            pageUrl: '/authorityMgmt/allChannels/roles/deleteRole.ajax',
            req: req,
    ...
    app.post('/authorityMgmt/allChannels/roles/getChannels.ajax', (req, res, next) => {
        let option = {
            pageUrl: '/authorityMgmt/allChannels/roles/getChannels.ajax',
            req: req,
            url: apiUrlList.getMenuData,
            timeout: 15000,
            json: true
    ...
    app.post('/authorityMgmt/allChannels/roles/saveRole.ajax', (req, res, next) => {
        ...
        if (req.body.readOnly === 'true') {
            params.menuList = params.menuList.map(value => {
                value.includes('@') ? value += 'R' : value += '@R';
                return value;
            });
        }
        let option = {
            pageUrl: '/authorityMgmt/allChannels/roles/saveRole.ajax',
            req: req,
...
function setChannelCheckedNode(channel, treeNodeUrls) {
    treeNodeUrls.forEach((item) => {
        if (item.menuId.toString().split('-')[0].replace(/@.*/g, '') == channel) {
