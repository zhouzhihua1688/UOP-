...
module.exports = function (app) {
    ...
    app.post('/messageCenter/templateMgmt/templateMgmt/search.ajax', (req, res, next) => {
        let option = {
            pageUrl: '/messageCenter/templateMgmt/templateMgmt/search.ajax',
            req: req,
    ...
    app.post('/messageCenter/templateMgmt/templateMgmt/query.ajax', (req, res, next) => {
        ...
        let option = {
            pageUrl: '/messageCenter/templateMgmt/templateMgmt/query.ajax',
            req: req,
    ...
    app.post('/messageCenter/templateMgmt/templateMgmt/del.ajax', (req, res, next) => {
        ...
        let option = {
            pageUrl: '/messageCenter/templateMgmt/templateMgmt/del.ajax',
            req: req,
    ...
    app.post('/messageCenter/templateMgmt/templateMgmt/add.ajax', (req, res, next) => {
        ...
        if(req.body.dataColor && req.body.dataColor !== '{}'){
            params.dataColor = req.body.dataColor;
        }
        else {
            params.dataColor = ''
        }
        ...
        else if (req.body.channelType == 6) {
            params.title = '';
            params.summary = '';
            params.wxTemplateType = req.body.wxTemplateType;
            params.url = '';
        ...
        let option = {
            pageUrl: '/messageCenter/templateMgmt/templateMgmt/add.ajax',
            req: req,
    ...
    app.post('/messageCenter/templateMgmt/templateMgmt/update.ajax', (req, res, next) => {
        ...
        let option = {
            pageUrl: '/messageCenter/templateMgmt/templateMgmt/update.ajax',
            req: req,
    ...
    app.post('/messageCenter/templateMgmt/templateMgmt/addByExcel.ajax', (req, res, next) => {
        try {
            ...
            function checkTemplate(item) {
                ...
                if (item.channelType == 1) {
                ...
                else if (item.channelType == 2 && item.pushType != 1) {
                    if (!item.title) {
                        return false;
                    }
                }
                    ...
                    if (!item.wxTemplateType ||
                        ...
                        item.wxTemplateType !== 'navUpdate' &&
                        item.wxTemplateType !== 'notice')) {
                        return false;
                    }
                }
                else if (item.channelType == 5) {
                    if (!item.wxTemplateType ||
                        (item.wxTemplateType !== 'highTrade' &&
                        item.wxTemplateType !== 'highService' &&
                        item.wxTemplateType !== 'highInvestment')) {
                        return false;
                    }
                }
                else if (item.channelType == 6) {
                    if (!item.wxTemplateType ||
                        (item.wxTemplateType !== 'report' &&
                        item.wxTemplateType !== 'id' &&
                        item.wxTemplateType !== 'review' &&
                        item.wxTemplateType !== 'unbind')) {
                        return false;
                    }
                }
                else {
            ...
            let finalData = ExcelData.map(item => {
                ...
                params.pushType = item.pushType;
                params.content = item.content;
                params.dataColor = item.dataColor ? item.dataColor : '';
                params.templateType = 0;
            ...
            function sendTemplate(item) {
                return new Promise((resolve, reject) => {
                    let option = {
                        pageUrl: '/messageCenter/templateMgmt/templateMgmt/addByExcel.ajax',
                        req: req,
                        operateType: 1, // operateType:操作类型 0:登录 1:新增 2:修改 3:删除 4:修改密码
                        url: apiUrlList.add,
                        body: item,
    ...
    app.post('/messageCenter/templateMgmt/templateMgmt/updateByExcel.ajax', (req, res, next) => {
        try {
            ...
            function sendTemplate(item) {
                return new Promise((resolve, reject) => {
                    let option = {
                        pageUrl: '/messageCenter/templateMgmt/templateMgmt/updateByExcel.ajax',
                        req: req,
                        operateType: 2, // operateType:操作类型 0:登录 1:新增 2:修改 3:删除 4:修改密码
                        url: apiUrlList.update,
                        body: item,
    ...
    app.get('/messageCenter/templateMgmt/templateMgmt/exportAll.ajax', (req, res, next) => {
        let option = {
            pageUrl: '/messageCenter/templateMgmt/templateMgmt/exportAll.ajax',
            req: req,
