...
module.exports = function (app) {
	...
	app.post('/messageCenter/auditMgmt/idCardHandle/getList.ajax', (req, res, next) => {
		...
		function getList() {
			return new Promise((resolve, reject) => {
				let params = req.body;
				let option = {
					// pageUrl: '/messageCenter/auditMgmt/idCardHandle/getList.ajax',
					req: req,
				...
				console.log(`/messageCenter/auditMgmt/idCardHandle/getList.ajax option:`, option.req ? {...option, req: '#'} : option);
				request.post(option, (error, response, body) => {
					...
					console.log(`/messageCenter/auditMgmt/idCardHandle/getList.ajax body: ******`);
					if (error) {
						reject({
							message: '数据获取失败'
						});
		...
		function getLocalserviceIdList() {
			return new Promise(resolve => {
				pool.getConnection((error, connection) => {
					...
					let SQL =
						`SELECT service_id,json_unquote(content->'$.serialNo') as content_id FROM ${tableName} WHERE delete_flag='F'`;
					SQL += ` AND status='1'`;
		...
		function getTranslate() {
			return new Promise((resolve, reject) => {
				...
				let option = {
					pageUrl: '/messageCenter/auditMgmt/idCardHandle/getList.ajax --getTranslate',
					req: req,
	...
	app.post('/messageCenter/auditMgmt/idCardHandle/getBase64.ajax', (req, res, next) => {
		...
		Promise.all(JSON.parse(req.body.imageInfo).map(custAttchmntItem => getImageByCustAttchmnt(custAttchmntItem))).then(
			imageArr => {
	...
	app.post('/messageCenter/auditMgmt/idCardHandle/serviceToLocalUpdate.ajax', (req, res, next) => {
		pool.getConnection((error, connection) => {
			...
			let SQL =
				`INSERT INTO ${tableName} (service_id,content,creator,operator,operate,necessary_remark,status) VALUES ('${serialNo}','${content}','${creator}','${operator}','${setOperate}','${necessaryRemark}',1)`;
			console.log('/messageCenter/auditMgmt/idCardHandle/serviceToLocalUpdate.ajax run business SQL: ******');
			connection.query(SQL, (error, results) => {
				console.log('/messageCenter/auditMgmt/idCardHandle/serviceToLocalUpdate.ajax run business SQL error:', error);
				console.log('/messageCenter/auditMgmt/idCardHandle/serviceToLocalUpdate.ajax run business SQL results:',
					results);
				connection.release();
				if (error) {
					return res.send({
	...
	app.post('/messageCenter/auditMgmt/idCardHandle/getLocalList.ajax', (req, res, next) => {
		pool.getConnection((error, connection) => {
			...
			let getTotalCount = new Promise((resolve, reject) => {
				let SQL = `SELECT COUNT(local_id) FROM ${tableName} WHERE delete_flag='F'`;
				if (req.body.custNo) {
					SQL += ` AND JSON_EXTRACT(content, '$.custNo')='${req.body.custNo}'`;
				}
				if (req.body.invNm) {
				...
				connection.query(SQL, function (error, results) {
					console.log('/messageCenter/auditMgmt/idCardHandle/getLocalList.ajax run SQL error', error);
					console.log('/messageCenter/auditMgmt/idCardHandle/getLocalList.ajax run SQL results', results);
					if (error) {
						return reject({
							message: '运行SQL语句失败'
						});
			...
			Promise.all([getTableList, getTotalCount]).then(([useData, total]) => {
				connection.release();
				res.send({
					error: 0,
					msg: '调用成功',
					data: {
						useData: useData,
						user: req.session.loginInfo.userid,
						total: total,
	...
	app.post('/messageCenter/auditMgmt/idCardHandle/revertLocalData.ajax', (req, res, next) => {
		pool.getConnection(function (error, connection) {
			...
			let SQL =
				`UPDATE ${tableName} SET delete_flag='T',operator='${req.session.loginInfo.userid}' WHERE local_id=${local_id} AND update_timestamp='${updateTime}'`;
			...
			connection.query(SQL, function (error, results) {
				...
				console.log('/messageCenter/auditMgmt/idCardHandle/revertLocalData.ajax run messageCenter SQL results:',
					results);
				connection.release();
				if (error) {
					return res.send({
